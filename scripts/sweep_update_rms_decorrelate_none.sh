...
# champion — NONE init (init_type=none, init_scale=4.0, ρ0.20, side_γ=0, lr5).
# Recipe mirrors scripts/sweep_poet_lie_orth_update_rms_none.sh (the run that produced the
# baseline), with ρ fixed at its optimum and λ swept instead. NOTE: `none` prefers a LOWER
# ρ (0.20, monotonic — §2.11), unlike mup/normalized (ρ0.30).
#
# Baseline to beat: val 3.4782 (POET_dev.md §2.6, run ufz71nt1). λ swept {0.25,0.50,0.75};
# mode symmetric (fires every step), renorm on (direction-only change), all layers
...
HELD="base/scale=60m training_regime=ablation_40x \
  optim.poet.init_type=none optim.poet.init_scale=4.0 optim.poet.scale=1.0 \
  optim.poet.lie_ortho_update_rms=0.20 optim.poet.lie_ortho_max_angle=0.024 \
...
for LAM in 0.25 0.50 0.75; do
  TAG="${LAM/./p}"                       # 0.25 -> 0p25
  NAME="urms_decorr_none_l${TAG}"
  echo ">>> ${NAME} (none s4, rho0.20, lambda=${LAM}) starting — baseline 3.4782"
...
echo "=== update-RMS decorrelation (NONE s4, ρ0.20) sweep complete: λ {0.25,0.50,0.75} vs baseline 3.4782 ==="
