...
# POET Lie-Orth update-RMS variant. Same harness as train_poet_lie_orth.sh, but
# selects experiment=optim/poet_lie_orth_update_rms and q_optimizer=lie_ortho_update_rms.
...
python -m launchers.train_megatron \
  ...
  "${SCHED_ARGS[@]}" \
  "cluster=h100_de" \
  "experiment=optim/poet_lie_orth_update_rms" \
