...
#       lss_mup_s1_ctrl  (mup α4, side_γ0.25) -> anchor 3.4745
#       lss_norm_s1_ctrl (normalized, side_γ0) -> anchor 3.4765
#   - 6 gentle-gain probes: each init × gain_lr_mult {0.1,0.03,0.01} (gain LR 5e-4/1.5e-4/5e-5)
# Each init uses its OWN champion side_γ (mup +0.25, normalized 0). init_scale=1.0 (the
# meaningful "optimal init + gain" A/B). g still inits at 1.0 (bit-exact baseline at step 0).
...
NORM="optim.poet.init_type=normalized optim.poet.init_scale=1.0 optim.poet.lie_ortho_update_rms_side_gamma=0.0"
...
build_arms() {  # "<name>\t<per-arm overrides>"
  ...
  printf '%s\t%s\n' lss_norm_s1_ctrl  "${NORM}"
  printf '%s\t%s\n' lss_norm_s1_m0p1  "${NORM} optim.poet.learnable_scale=true optim.poet.gain_lr_mult=0.1"
  printf '%s\t%s\n' lss_norm_s1_m0p03 "${NORM} optim.poet.learnable_scale=true optim.poet.gain_lr_mult=0.03"
  printf '%s\t%s\n' lss_norm_s1_m0p01 "${NORM} optim.poet.learnable_scale=true optim.poet.gain_lr_mult=0.01"
}
