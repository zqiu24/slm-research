...
# Dev variant of train_poet.sh: same harness, but defaults to the tiny 60m
# scale (configs/base/scale/60m.yaml, hidden=512, ~61M non-embedding params,
# every weight dim divisible by 512 so POET block_size=512 works) for fast
# local iteration instead of 300m. Untied embeddings are forced on (overridable).
# Everything else is identical to train_poet.sh, and any "$@" override still wins.
