...
# poet_lie_head variant: same harness as train_poet_lie.sh, but uses
# experiment=optim/poet_lie_head — POET x Pion Lie-algebra momentum
# (q_optimizer=lie_algebra, element-wise v) WITH head-aligned attention rotation
# (head_aligned_attn=true): q/k/v/o rotate their head-structured side per head
# (block_size=head_dim, identity Psi, no cross-head mixing); the residual side
# stays a normal POET rotation. Requires unfused q/k/v (config sets unfuse_qkv).
...
python -m launchers.train_megatron \
  ...
  "${SCHED_ARGS[@]}" \
  "cluster=h100_de" \
  "experiment=optim/poet_lie_head" \
