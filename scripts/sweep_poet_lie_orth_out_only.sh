#!/usr/bin/env bash
# POET one-sided (OUT-ONLY) GRID sweep — COSINE scheduler (cosine_poet, 1% floor).
# Run on one node (24 sequential runs, each grabs the whole node and blocks):
#   bash scripts/sweep_poet_lie_orth_out_only.sh
#
# Sibling: scripts/sweep_poet_lie_orth_in_only.sh — SAME 24-cell grid, IN side.
...
#   single_step_x=TRUE, single_step_x_one_sided=out (FIXED side, NOT alternating),
...
PREFIX="outonly"
TRAIN="scripts/train_poet_lie_orth_out_only.sh"
...
echo "=== POET out-only grid sweep complete (24 runs) ==="
