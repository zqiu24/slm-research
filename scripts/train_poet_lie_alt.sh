...
# poet_lie_alt variant: same harness as train_poet_lie.sh, but uses
# experiment=optim/poet_lie_alt — POET x Pion Lie-algebra momentum
# (q_optimizer=lie_algebra) WITH the alternating single-sided update (§6,
# lie_alternating=true): write one rotation side per step (out even / in odd),
# momentum on both sides. Single-step (merge_period=1), reinit_period=-1,
# block_count=1. Any "$@" override still wins.
