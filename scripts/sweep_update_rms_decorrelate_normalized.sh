...
# champion — NORMALIZED init (init_type=normalized, init_scale=2.0, ρ0.30, side_γ=0, lr5).
# Recipe mirrors scripts/sweep_poet_lie_orth_update_rms_normalized.sh (the run that produced
# the baseline), with ρ fixed at its optimum and λ swept instead.
#
# Baseline to beat: val 3.4765 (POET_dev.md §2.6, run ybykexu1). λ swept {0.25,0.50,0.75};
...
HELD="base/scale=60m training_regime=ablation_40x \
  optim.poet.init_type=normalized optim.poet.init_scale=2.0 optim.poet.scale=1.0 \
...
for LAM in 0.25 0.50 0.75; do
  TAG="${LAM/./p}"                       # 0.25 -> 0p25
  NAME="urms_decorr_norm_l${TAG}"
  echo ">>> ${NAME} (normalized s2, rho0.30, lambda=${LAM}) starting — baseline 3.4765"
...
echo "=== update-RMS decorrelation (NORMALIZED s2, ρ0.30) sweep complete: λ {0.25,0.50,0.75} vs baseline 3.4765 ==="
