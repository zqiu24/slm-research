...
# "Full" dev variant of train_poet_dev.sh. Identical harness and tiny 60m dev
# scale, but with two POET changes baked in:
#   * optim.poet.block_count=1 — a single block spanning each side, i.e. a FULL
#     orthogonal rotation per linear instead of the block-diagonal default.
#     block_count takes precedence over the config's block_size (see
#     src/utils/megatron_args.py), and divisor=1 means no layer is skipped for
#     divisibility.
#   * optim.poet.merge_period=0 — disables the periodic merge AND the Adam
#     momentum reset (the poet_merge_step hook short-circuits when gap<=0), so
#     oft_R rotates continuously with no merge/reset cycle.
# Everything else matches train_poet_dev.sh, and any "$@" override still wins
# (later dotlist entries override earlier ones).
