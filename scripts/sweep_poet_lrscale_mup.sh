...
# (mup norm axis is mup_alpha; init_scale stays at its 1.0 default.)
#
# Baseline to beat: init_mup_a400_c6 = val/loss 3.4816 (the grid center lr4/ps50 reproduces it).
#   bash scripts/sweep_poet_lrscale_mup.sh        # default GPU 0-3 (dp=4, global_batch=1024), sequential
set -uo pipefail
...
echo "=== POET lr x scale sweep (init mup_normalized, alpha 4/c6) complete: 12 runs (baseline 3.4816) ==="
