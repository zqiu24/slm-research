#!/usr/bin/env bash
# Node 2 of the 16-arm learnable-scale A/B: mup_normalized α4, init_scale=4.0 (norm-recovery).
...
ARMS=(ls_mup_s4_m0p1 ls_mup_s4_m0p25 ls_mup_s4_m0p5 ls_mup_s4_m1p0)
echo ">>> node2 starting ${#ARMS[@]} arms: ${ARMS[*]}"
for arm in "${ARMS[@]}"; do
  "${SWEEP}" "${arm}"
done
echo "<<< node2 done: ${ARMS[*]}"
