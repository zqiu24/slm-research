#!/usr/bin/env bash
# POET lr x poet-scale sweep — init_type=none, FIXED best init_scale=4.0 (§2.9, row_rms ~0.064).
#
# The §2.9 init sweep fixed the frozen-base NORM optimum (none @ init_scale 4.0 / c6 = 3.4804).
# This sweep holds that norm + angle (c6) and instead perturbs the TWO optimizer levers around
# the champion (lr 4e-3, poet.scale 0.5):
...
# (Nesterov + lie_b1=0.95 come from the poet_lie_orth experiment config; re-stated below for record.)
#
# Baseline to beat: hi_none_s4_c6 = val/loss 3.4804 (the grid center lr4/ps50 reproduces it).
#   bash scripts/sweep_poet_lrscale_none.sh        # full 8-GPU node (dp=8, global_batch=1024), sequential
set -uo pipefail
...
# Champion recipe; init NORM (init_scale=4.0) and ANGLE (c6) PINNED. lr and poet.scale vary per cell.
...
for lr in "${LRS[@]}"; do
  for sc in "${SCALES[@]}"; do
    run "lrsc_none_${lr%%:*}_${sc%%:*}" "${lr##*:}" "${sc##*:}"
  done
done

echo "=== POET lr x scale sweep (init none, scale 4.0/c6) complete: 15 runs (baseline 3.4804) ==="
