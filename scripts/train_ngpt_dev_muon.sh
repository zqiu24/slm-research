...
# Dev launcher for nGPT-with-Muon (experiment=arch/ngpt_muon): the nGPT
# hypersphere architecture trained with the vendored Kimi/Moonlight Muon
# optimizer instead of nGPT's native ngpt_adamw. Mirrors train_ngpt_dev.sh
# line-for-line EXCEPT for the experiment, so the architecture-vs-optimizer
# ablation is directly comparable on the same llama3 backbone and training
# regime: tiny 60m scale (configs/base/scale/60m.yaml, hidden=512, ~61M
# non-embedding params), 40x-tokens-per-param regime, gbs 1024 / mbs 128, local
# transformer impl, untied embeddings, dev wandb project. Any "$@" override
# still wins. See configs/experiments/arch/ngpt_muon.yaml for how the
# nGPT architecture and the Muon optimizer are composed.
