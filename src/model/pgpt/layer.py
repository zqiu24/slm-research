...
* `PGPTBlock` — the pure-PyTorch parity-oracle block now lives in
  `src/model/ngpt/block.py`. It is re-exported here so existing
  `from src.model.pgpt.layer import PGPTBlock` callers keep working.

* `PGPTTransformerLayer` — subclass of Megatron's `TransformerLayer`
...
from src.model.pgpt.attention import QKHyperNorm  # noqa: F401  (used via spec)
from src.model.pgpt.block import (  # _residual_blend reused by forward
    PGPTBlock,  # noqa: F401  -- re-exported for back-compat
    _residual_blend,
)
from src.model.pgpt.mlp import PGPTMLPBody  # noqa: F401  (used via spec)
...
class PGPTTransformerLayer(TransformerLayer):
