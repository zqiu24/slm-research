...
PGPTMLPBody is a CPU-runnable pure-PyTorch module that matches the
reference's MLP fragment. It is what PGPTTransformerLayer instantiates
when the layer spec wires `mlp=PGPTMLPBody`. We deliberately do NOT
...
class PGPTMLP(PGPTMLPBody):
    ...
    ``PGPTTransformerLayer.forward`` unpacks the tuple.
    """
