...
# Model1 30B-A5B Training Script with YAML Configuration
# This script uses Model1-30B-A5B.yaml for model arguments
...
export GLOO_SOCKET_IFNAME=eth0 # must use for 128GPUs
...
# ==============================================================================
GPUS_PER_NODE=8
NUM_NODES=${1:-1}  # Default to 1 node if not provided
...
MBS=4       # Micro batch size (reduced for 30B model with limited memory)
GBS=2560    # Global batch size (optimized for 30B model)
SEQ_LEN=4096  # Sequence length
EVAL_STEP=1000000000 ## to 200 ??
EVAL_ITERS=0
SAVE_STEP=1000 ## real: 1000
MONITOR_STEP=100 ## real: 100

# Training-specific parameters (not in YAML)
TRAIN_ITERS=95368 # 1T
LR_WARMUP_ITERS=2000 # ??
LR=4e-4 ## ??     # Lower LR for 30B model
...
SAVE_DIR=$MAIN_DIR/experiments ## ??

## data and tokenizer
DATA_NAME=DeepSeek-V3+dolma3-Shuffle-BFD ## ??
VALID_DATA_NAME=dolma3_Shuffle_BFD
DATA=/public/Datasets/04_mixing_lists/DeepSeek-V3+dolma3-Shuffle-BFD.list ## ??
TOKENIZER=/public/Datasets/00_tokenizers/DeepSeek-V3 ## ??

RUNTIME=$(date +%m%d%H%M)
DATE=$(date +%Y%m%d)
export WANDB_PROJECT="your wandb project name" ## ??
export WANDB_EXP_NAME="MODEL30bA5b_bf16_seqlen${SEQ_LEN}_node${NUM_NODES}_data${VALID_DATA_NAME}_mbs${MBS}_gbs${GBS}_TP${TP}_PP${PP}_CP${CP}_EP${EP}_eval${EVAL_STEP}_save${SAVE_STEP}_${RUNTIME}"
...
DATA_CACHE_PATH="$MAIN_DIR/data_cache/benchmark_cache_model_30b_${DATA_NAME}"
...
# ==============================================================================
YAML_CONFIG="$SCRIPT_DIR/model_args/Model1-30B-A5B.yaml"
...
if [ -n "${WANDB_PROJECT:-}" ]; then
    WANDB_ARGS=(
        ...
        "--wandb-exp-name" "${WANDB_EXP_NAME:-model1_30b_a5b}"
...
# Validation
# ==============================================================================
export PYTHONPATH=/your/path/to/Megatron-LM
cd $PYTHONPATH
...
# ==============================================================================
echo "==============================================="
echo "Starting Model1 30B-A5B Training"
echo "==============================================="
