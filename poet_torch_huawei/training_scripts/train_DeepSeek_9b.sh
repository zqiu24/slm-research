...
# DeepSeek 9B Training Script with YAML Configuration
# This script uses DeepSeek-9B.yaml for model arguments
...
MBS=8       # Micro batch size - H200 has 141GB, can use larger MBS
GBS=512     # Global batch size (optimized for 9B model)
...
LR=5.9e-4      # Slightly lower LR for 9B model
...
export WANDB_EXP_NAME="DPSK9b_bf16_seqlen${SEQ_LEN}_node${NUM_NODES}_data${VALID_DATA_NAME}_mbs${MBS}_gbs${GBS}_TP${TP}_PP${PP}_CP${CP}_EP${EP}_eval${EVAL_STEP}_save${SAVE_STEP}_${RUNTIME}"
...
DATA_CACHE_PATH="$MAIN_DIR/data_cache/benchmark_cache_deepseek_9b_${DATA_NAME}"
...
# ==============================================================================
YAML_CONFIG="$SCRIPT_DIR/model_args/DeepSeek-9B.yaml"
...
if [ -n "${WANDB_PROJECT:-}" ]; then
    WANDB_ARGS=(
        ...
        "--wandb-exp-name" "${WANDB_EXP_NAME:-deepseek_9b}"
...
# ==============================================================================
echo "==============================================="
echo "Starting DeepSeek 9B Training"
echo "==============================================="
