...
## main dir
MAIN_DIR="${MAIN_DIR:-/public/Workspace}"
...
DATA="${DATA:-/public/Datasets/04_mixing_lists/DeepSeek-V3+dolma3_mix-6T+Shuffle+Stack.part0.list}"
VALID_DATA="${VALID_DATA:-/public/Datasets/04_mixing_lists/DeepSeek-V3+dolma3_mix-6T+Shuffle+Stack.part1.list}"
TOKENIZER="${TOKENIZER:-/public/Datasets/00_tokenizers/DeepSeek-V3}"

RUNTIME=$(date +%m%d%H%M)
DATE=$(date +%Y%m%d)
export WANDB_PROJECT="H200_baseline_3Bv2" ## ??
...
export WANDB_API_KEY="${WANDB_API_KEY:?Error: WANDB_API_KEY not set. Please export it from your environment.}"
...
# Persist logs next to this launch script while keeping console output.
LOG_TIMESTAMP=$(date +"%Y-%m-%d-%H-%M-%S")
LOG_FILE="${SCRIPT_DIR}/train_output_${LOG_TIMESTAMP}.log"
...
# ==============================================================================
YAML_CONFIG="$SCRIPT_DIR/model_args/DeepSeek-3Bv2-sandwich.yaml"
...
export PYTHONPATH="${PYTHONPATH:-$(dirname "$SCRIPT_ABS_DIR")}"
cd $PYTHONPATH
