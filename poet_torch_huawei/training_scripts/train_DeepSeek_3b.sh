...
MBS=8              # Micro batch size (not in YAML, script-specific)
GBS=2560 # for 4nodes 1T else GBS 1node 200B set to 320            # Global batch size (not in YAML, script-specific)
...
export WANDB_EXP_NAME="DPSK3b_bf16_seqlen${SEQ_LEN}_node${NUM_NODES}_data${VALID_DATA_NAME}_mbs${MBS}_gbs${GBS}_TP${TP}_PP${PP}_CP${CP}_EP${EP}_eval${EVAL_STEP}_save${SAVE_STEP}_${RUNTIME}"
...
DATA_CACHE_PATH="$MAIN_DIR/data_cache/benchmark_cache_deepseek_3b_${DATA_NAME}"
...
# ==============================================================================
YAML_CONFIG="$SCRIPT_DIR/model_args/DeepSeek-3B.yaml"
