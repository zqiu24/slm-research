...
# DeepSeek 27B Training Script with YAML Configuration
# This script uses DeepSeek-27B.yaml for model arguments
...
MBS=4       # Micro batch size - H200 has 141GB, but 27B is large
GBS=1280   # Global batch size (optimized for 27B model)
...
LR=4e-4      # Lower LR for larger 27B model
...
export WANDB_EXP_NAME="DPSK27b_bf16_seqlen${SEQ_LEN}_node${NUM_NODES}_data${VALID_DATA_NAME}_mbs${MBS}_gbs${GBS}_TP${TP}_PP${PP}_CP${CP}_EP${EP}_eval${EVAL_STEP}_save${SAVE_STEP}_${RUNTIME}"
...
DATA_CACHE_PATH="$MAIN_DIR/data_cache/benchmark_cache_deepseek_27b_${DATA_NAME}"
...
# ==============================================================================
YAML_CONFIG="$SCRIPT_DIR/model_args/DeepSeek-27B.yaml"
...
if [ -n "${WANDB_PROJECT:-}" ]; then
    WANDB_ARGS=(
        ...
        "--wandb-exp-name" "${WANDB_EXP_NAME:-deepseek_27b}"
...
# ==============================================================================
echo "==============================================="
echo "Starting DeepSeek 27B Training"
echo "==============================================="
