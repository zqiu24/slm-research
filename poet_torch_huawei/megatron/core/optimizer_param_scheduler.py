...
class OptimizerParamScheduler:
    ...
    Args:
        ...
        lr_wsd_decay_style (str, optional): decay style for learning rate during weight decay decay
            steps
        lr_decay_step_ratio (list of float, optional): training progress ratios at which lr steps
            down. Used with lr_decay_style='step'.
        lr_decay_step_coeff (list of float, optional): lr coefficient values at each step point.
            Used with lr_decay_style='step'.
    ...
    def __init__(
        ...
        lr_decay_step_ratio: Optional[list] = None,
        lr_decay_step_coeff: Optional[list] = None,
    ) -> None:
        ...
        self.lr_decay_step_ratio = lr_decay_step_ratio
        self.lr_decay_step_coeff = lr_decay_step_coeff
        if self.lr_decay_style == "step":
            assert self.lr_decay_step_ratio is not None, \
                "--lr-decay-step-ratio must be set when using step decay style"
            assert self.lr_decay_step_coeff is not None, \
                "--lr-decay-step-coeff must be set when using step decay style"
            assert len(self.lr_decay_step_ratio) == len(self.lr_decay_step_coeff), \
                "--lr-decay-step-ratio and --lr-decay-step-coeff must have the same length"
            assert all(0 < r < 1 for r in self.lr_decay_step_ratio), \
                "--lr-decay-step-ratio values must be in (0, 1)"
            assert self.lr_decay_step_ratio == sorted(self.lr_decay_step_ratio), \
                "--lr-decay-step-ratio must be sorted in ascending order"
    ...
    def get_lr(self, param_group: dict) -> float:
        ...
        elif self.lr_decay_style == 'step':
            progress = float(self.num_steps) / float(self.lr_decay_steps)
            coeff = 1.0
            for ratio, c in zip(self.lr_decay_step_ratio, self.lr_decay_step_coeff):
                if progress >= ratio:
                    coeff = c
