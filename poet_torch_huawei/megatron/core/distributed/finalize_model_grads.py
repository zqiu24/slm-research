...
from ..transformer.moe.moe_utils import get_updated_expert_bias, save_reduced_tokens_for_max_vio
...
def _update_router_expert_bias(model: List[torch.nn.Module], config: TransformerConfig):
    ...
    Also saves the all-reduced tokens_per_expert for MaxVio metric computation,
    avoiding a redundant all-reduce in the logging path.
    """
    ...
    for model_chunk in model:
        for module in get_attr_wrapped_model(model_chunk, 'modules')():
            if hasattr(module, 'expert_bias'):
                ...
                layer_number_list.append(module.layer_number)
    ...
    # Piggyback on the already TP×CP×DP all-reduced stacked_tokens_per_expert
    # to save for MaxVio computation, avoiding a second all-reduce.
    num_layers = config.num_layers
    if getattr(config, 'mtp_num_layers', None) is not None:
        num_layers += config.mtp_num_layers
    save_reduced_tokens_for_max_vio(
        stacked_tokens_per_expert, layer_number_list, num_layers, config.num_moe_experts
    )
