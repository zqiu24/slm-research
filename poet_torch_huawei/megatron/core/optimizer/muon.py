...
def zeropower_via_newtonschulz5(G, steps):
    ...
    if steps < 1 or steps > len(coeffs):
        raise ValueError(f"Muon Newton-Schulz steps must be in [1, {len(coeffs)}], got {steps}")

    # This implementation uses a fixed coefficient schedule, so honoring
    # `steps` means taking the requested prefix of that schedule.
...
class MuonDistMeta:
    ...
    def __init__(self, buffer_idx: int, bucket_idx: int, shape: torch.Size, global_range: Tuple[int, int], tp_split_dim: int, ns_shape):
        self.buffer_idx = buffer_idx
        self.bucket_idx = bucket_idx
        self.shape = shape
        self.global_range = global_range
        self.tp_split_dim = tp_split_dim
        self.ns_shape = ns_shape
...
def adjust_lr_wd_for_muon(lr, matched_adamw_rms, param_shape):
    if len(param_shape) < 2:
        raise ValueError(f"Muon expects at least 2D shapes for LR adjustment, got {param_shape}")

    # `ns_shape` may be batched (e.g. MLA head groups), but the matched-RMS
    # scaling should use the matrix dimensions that Newton-Schulz acts on.
    A, B = param_shape[-2:]
        ...
        for group in self.param_groups:
            ...
            for p in params:
                ...
                if ns_shape is not None:
                    origin_shape = ns_input.shape
                    ns_input = ns_input.view(ns_shape)

                # calc update
                update = zeropower_via_newtonschulz5(ns_input, steps=ns_steps)

                if ns_shape is not None:
                    update = update.view(origin_shape)
                ...
                adjusted_lr = adjust_lr_wd_for_muon(lr, matched_adamw_rms, ns_input.shape if ns_shape is None else ns_shape)
