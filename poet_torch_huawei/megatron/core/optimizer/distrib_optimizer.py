...
class DistributedOptimizer(MixedPrecisionOptimizer):
    ...
    def _build_model_and_main_param_groups(
        ...
        # Muon needs a per-shard-param MuonDistMeta describing the param's
        # global-buffer placement + tp split info so it can do distributed NS.
        dist_metas = {}
        ...
        for group_range in opt_group_ranges:
            ...
            for model_param in group_range["params"]:
                ...
                # Build MuonDistMeta for this model param. Safe to build for
                # all optimizers; it is only consumed by Muon.
                param_world_indexes = param_gbuf_ranges["world_indexes"]
                tp_split_dim = (
                    getattr(model_param, 'partition_dim', -1)
                    if getattr(model_param, 'tensor_model_parallel', False)
                    else -1
                )
                ns_shape = getattr(model_param, 'ns_shape', None)
                ...
                if model_param.type() in ['torch.cuda.HalfTensor', 'torch.cuda.BFloat16Tensor']:
                    ...
                    # Muon indexes dist_metas by the sharded main param that
                    # the inner optimizer actually sees.
    ...
    def __init__(
        ...
        if isinstance(self.optimizer, HybridDeviceOptimizer):
            ...
            # Enable Muon's distributed mode now that sharded main params and
            # the gbuf layout are known.
            if isinstance(self.optimizer, Muon):
                gbuf_sizes = [
                    [
                        (bucket.grad_data.numel(), bucket.offset)
    ...
    def load_state_dict(self, state_dict):
        ...
        ** Note: Torch optimizer's state structure. **
        ...
        def make_needed_groups(param_group):
            ...
            # Muon appends an extra group that shares (wd_mult, lr_mult,
            # is_expert_parallel, is_decoupled_lr) with an AdamW group, so
            # also key on `use_muon` to keep them distinct.
            if param_group.get('use_muon', False):
                needed_groups.append(param_group.get('use_muon'))
        ...
        for param_group in state_dict["optimizer"]["param_groups"]:
            ...
            if needed_groups in param_groups_map:
                raise ValueError(
                    f"key {needed_groups} exists in param_groups_map, "
                    f"modify function `make_needed_groups` to make `needed_groups` unique."
                )
                ...
                for gbuf_range_map_for_all_buckets in gbuf_range_maps.values():
                    for gbuf_range_map in gbuf_range_map_for_all_buckets:
                        for model_param, param_range_map in gbuf_range_map["param_map"].items():
                            ...
                            # Muon internally uses either `muon_buffer` (muon groups)
                            # or `adamw_exp_avg[_sq]` (adam-fallback groups). We alias
                            # them to the same storage used by exp_avg[_sq] so
                            # load_parameter_state writes land in the right place.
                            if isinstance(self.optimizer, Muon):
    ...
    def _get_main_param_and_optimizer_states(self, model_param):
        ...
        # Normalize Muon's internal state names into the standard
        # exp_avg / exp_avg_sq keys used by the checkpoint format.
        if isinstance(self.optimizer, Muon):
            use_muon = self.optimizer.param_groups[group_index].get("use_muon", False)
            if use_muon:
                # Muon groups only have `muon_buffer`; there is no second moment.
                tensors["exp_avg"] = tensors.pop("muon_buffer")
                tensors["exp_avg_sq"] = torch.zeros_like(tensors["param"])
                tensors.pop("adamw_exp_avg", None)
                tensors.pop("adamw_exp_avg_sq", None)
            else:
                tensors["exp_avg"] = tensors.pop("adamw_exp_avg")
                tensors["exp_avg_sq"] = tensors.pop("adamw_exp_avg_sq")
                tensors.pop("muon_buffer", None)
        return tensors
    ...
    def _set_main_param_and_optimizer_states(self, model_param, tensors):
        ...
        if self.config.use_precision_aware_optimizer_no_fp8_or_ds_fp8:
            ...
            # Reverse mapping for Muon: checkpoint stores exp_avg / exp_avg_sq
            # but the optimizer state keys are muon_buffer (muon group) or
            # adamw_exp_avg[_sq] (adam-fallback group).
            if isinstance(self.optimizer, Muon):
                use_muon = self.optimizer.param_groups[group_index].get("use_muon", False)
                if use_muon:
                    if "exp_avg" in tensors and "muon_buffer" not in tensors:
                        tensors = {**tensors, "muon_buffer": tensors["exp_avg"]}
                else:
                    if "exp_avg" in tensors and "adamw_exp_avg" not in tensors:
                        tensors = {**tensors, "adamw_exp_avg": tensors["exp_avg"]}
                    if "exp_avg_sq" in tensors and "adamw_exp_avg_sq" not in tensors:
                        tensors = {**tensors, "adamw_exp_avg_sq": tensors["exp_avg_sq"]}
