...
def _get_param_groups(
    ...
    params_map = {}
    # When using Muon, 2D linear weights (non-embedding, non-bias) go into a
    # separate map keyed by (wd_mult, lr_mult, is_expert_parallel).
    muon_params_map = {}
    for model_chunk in model_chunks:
        for name, param in model_chunk.named_parameters():
            ...
            # Dispatch to Muon if this is a ">=2D" linear weight (non-embedding,
            # non-bias). Everything else (biases, norms, embeddings, output
            # layer) stays on AdamW.
            ...
            if muon_flag:
                muon_key = (wd_mult, _lr_mult, is_expert_parallel)
                if muon_key not in muon_params_map:
                    muon_params_map[muon_key] = []
                muon_params_map[muon_key].append(param)
                log_single_rank(
                    logger,
                    logging.INFO,
                    f"Muon: {name} {tuple(param.shape)} "
                    f"wd={not bool(wd_mult == 0.0)} "
                    f"tp={getattr(param, 'tensor_model_parallel', False)} "
                    f"partition_dim={getattr(param, 'partition_dim', -1)} "
                    f"ns_shape={getattr(param, 'ns_shape', None)}",
                )
                continue
    ...
    def _align_param_group_keys_across_ranks(keys):
        keys = list(keys)
    ...
    params_key = _align_param_group_keys_across_ranks(params_map.keys())
    ...
    # Append Muon param groups (one per unique (wd_mult, lr_mult, is_expert_parallel)).
    # These are appended after the AdamW groups so the `assert` above is not triggered
    # (the extra `use_muon` key would otherwise violate param_group_identifier_keys).
    muon_params_key = _align_param_group_keys_across_ranks(muon_params_map.keys())
    for wd_mult, _lr_mult, is_expert_parallel in muon_params_key:
        params = muon_params_map.get((wd_mult, _lr_mult, is_expert_parallel), [])
        muon_group = {
            'params': params,
            'wd_mult': wd_mult,
            'lr_mult': _lr_mult,
