...
class TransformerLayer(MegatronModule, BaseTransformerLayer):
    ...
    def __init__(
        ...
        # [Module 3.5: Post SelfAttention Sandwich Norm]
        if self.config.use_sandwich_norm:
            sandwich_norm_cls = _get_sandwich_norm_impl()
            self.post_self_attn_layernorm = sandwich_norm_cls(
                config=self.config,
                hidden_size=self.config.hidden_size,
                eps=self.config.layernorm_epsilon,
            )
        ...
        # [Module 9.5: Post MLP Sandwich Norm]
        if self.config.use_sandwich_norm:
            sandwich_norm_cls = _get_sandwich_norm_impl()
            self.post_mlp_layernorm = sandwich_norm_cls(
                config=self.config,
                hidden_size=self.config.hidden_size,
                eps=self.config.layernorm_epsilon,
            )
        ...
        # Apply norm weight initialization scale
        if self.config.attn_pre_norm_scale != 1.0:
            if not isinstance(self.input_layernorm, IdentityOp):
                self.input_layernorm.weight.data.mul_(self.config.attn_pre_norm_scale)
            elif hasattr(self.self_attention, 'linear_qkv') and hasattr(
                self.self_attention.linear_qkv, 'layer_norm_weight'
            ):
                self.self_attention.linear_qkv.layer_norm_weight.data.mul_(
                    self.config.attn_pre_norm_scale
                )
        if self.config.ffn_pre_norm_scale != 1.0:
            if not isinstance(self.pre_mlp_layernorm, IdentityOp):
                self.pre_mlp_layernorm.weight.data.mul_(self.config.ffn_pre_norm_scale)
            elif hasattr(self.mlp, 'linear_fc1') and hasattr(
                self.mlp.linear_fc1, 'layer_norm_weight'
            ):
                self.mlp.linear_fc1.layer_norm_weight.data.mul_(
                    self.config.ffn_pre_norm_scale
                )
        if self.config.use_sandwich_norm:
            if self.config.attn_post_norm_scale != 1.0:
                self.post_self_attn_layernorm.weight.data.mul_(self.config.attn_post_norm_scale)
            if self.config.ffn_post_norm_scale != 1.0:
                self.post_mlp_layernorm.weight.data.mul_(self.config.ffn_post_norm_scale)
    ...
    def _forward_attention(
        ...
        *,
        ...
        if self.config.use_sandwich_norm:
            ...
            attention_output_with_bias = (attn_out, attention_output_with_bias[1])
    ...
    def _forward_mlp(self, hidden_states, inference_context=None):
        ...
        if self.config.use_sandwich_norm:
            ...
            mlp_output_with_bias = (mlp_out, mlp_output_with_bias[1])
