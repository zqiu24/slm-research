...
class SelfAttention(Attention):
    ...
    def __init__(
        ...
        self.split_qkv_projections = bool(getattr(self.config, 'poet_split_qkv', False))
        if self.split_qkv_projections:
            self.linear_q = build_module(
                submodules.linear_q if submodules.linear_q is not None else submodules.linear_qkv,
                self.config.hidden_size,
                self.query_projection_size,
                config=self.config,
            ...
            self.linear_k = build_module(
                submodules.linear_k if submodules.linear_k is not None else submodules.linear_qkv,
                self.config.hidden_size,
            ...
            self.linear_v = build_module(
                submodules.linear_v if submodules.linear_v is not None else submodules.linear_qkv,
                self.config.hidden_size,
                self.kv_projection_size,
                config=self.config,
                init_method=self.config.init_method,
    ...
    def _backward_qkv_proj(self):
        ...
        if self.split_qkv_projections:
            self.linear_q.backward_dw()
            self.linear_k.backward_dw()
            self.linear_v.backward_dw()
        else:
    ...
    def set_for_recompute_input_layernorm(self):
        ...
        if self.split_qkv_projections:
            set_save_original_input(self.linear_q)
            set_save_original_input(self.linear_k)
            set_save_original_input(self.linear_v)
        else:
