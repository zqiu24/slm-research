...
class MTPLossLoggingHelper:
    ...
    def save_acc_to_tracker(
        ...
        tracker["acc_count"][layer_number] += num_token.detach()
        tracker["reduce_group"] = reduce_group
        tracker["avg_group"] = avg_group
    ...
    def reduce_loss_in_tracker():
        ...
        if "acc_values" in tracker:
            ...
            acc_count = tracker["acc_count"]
            # Reduce mtp acc across ranks using weighted average:
            # First reduce (SUM) both numerator (correct predictions) and denominator (token count),
            # then divide to get accurate weighted average across all ranks.
            if tracker.get('reduce_group') is not None:
                torch.distributed.all_reduce(acc_values, group=tracker.get('reduce_group'))
                torch.distributed.all_reduce(acc_count, group=tracker.get('reduce_group'))
            if tracker.get('avg_group') is not None:
                torch.distributed.all_reduce(
                    acc_values, group=tracker['avg_group'], op=torch.distributed.ReduceOp.SUM
                )
                torch.distributed.all_reduce(
                    acc_count, group=tracker['avg_group'], op=torch.distributed.ReduceOp.SUM
                )
            # Store the reduced values back to tracker for use in track_mtp_metrics
            tracker["acc_values"] = acc_values
            tracker["acc_count"] = acc_count
