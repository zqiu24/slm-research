...
class MLASelfAttention(MultiLatentAttention):
    ...
    def __init__(
        ...
        if self.config.q_lora_rank is None:
            ...
            # When training with Muon, the q_up projection is conceptually a
            # [q_lora_rank, num_heads * q_head_dim] block-diagonal matrix; we
            # reshape it to [num_head_groups, q_lora_rank, heads/g * q_head_dim]
            # before Newton-Schulz so orthogonalization happens per head-group
            # rather than across all heads. See Moonshot's Muon recipe.
            from megatron.training import get_args
            args = get_args()
            if args.optimizer == 'muon' and args.muon_ns_num_head_groups > 0:
                muon_ns_num_head_groups = args.muon_ns_num_head_groups
                assert self.config.num_attention_heads % muon_ns_num_head_groups == 0, (
                    f"num_attention_heads={self.config.num_attention_heads} must be "
                    f"divisible by muon_ns_num_head_groups={muon_ns_num_head_groups}"
                )
                setattr(
                    self.linear_q_up_proj.weight,
                    'ns_shape',
                    (
                        muon_ns_num_head_groups,
                        self.config.q_lora_rank,
                        self.config.num_attention_heads * self.q_head_dim
                        // muon_ns_num_head_groups,
                    ),
        ...
        # See the q_up_proj comment above for why we reshape the kv_up_proj
        # weight into a head-grouped 3D tensor before Newton-Schulz.
        from megatron.training import get_args
        args = get_args()
        if args.optimizer == 'muon' and args.muon_ns_num_head_groups > 0:
            muon_ns_num_head_groups = args.muon_ns_num_head_groups
