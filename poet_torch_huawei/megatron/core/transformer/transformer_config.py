...
class TransformerConfig(ModelParallelConfig):
    ...
    """If True, add post-layernorm after attn/mlp output, before residual connection."""

    attn_pre_norm_scale: float = 1.0
    """Scale factor for attn pre-layernorm (input_layernorm) weight initialization."""

    attn_post_norm_scale: float = 1.0
    """Scale factor for attn post-layernorm weight initialization. Only used with sandwich norm."""

    ffn_pre_norm_scale: float = 1.0
    """Scale factor for ffn pre-layernorm (pre_mlp_layernorm) weight initialization."""

    ffn_post_norm_scale: float = 1.0
    """Scale factor for ffn post-layernorm weight initialization. Only used with sandwich norm."""
    ...
    ####################
    # Native Sparse Attention (NSA / DeepSeek-style)
    ####################
    use_native_sparse_attention: bool = False
    """Whether to use Native Sparse Attention (NSA) for long-context:
    coarse compression + sliding window (selection branch removed)."""

    nsa_compress_block_size: int = 64
    """Block size for coarse-grained token compression in NSA."""

    nsa_compress_block_sliding_stride: int = 32
    """Stride for compression block sliding in NSA. Must be <= nsa_compress_block_size."""

    nsa_num_compressed_mem_kv: int = 4
    """Number of compressed memory KV slots (global tokens) in NSA."""

    nsa_compress_mlp_expand_factor: float = 2.0
    """Hidden expansion factor for compression MLP in NSA."""

    nsa_swa_window_size: int = 0
    """If > 0, concatenate compressed KV (no RoPE) with a sliding-window KV branch
    (RoPE on keys) in NSA attention. 0 keeps compress-only behavior."""

    nsa_swa_only: bool = False
    """When True, use sliding-window attention only (no compression).
    Requires nsa_swa_window_size > 0. The compress MLP is not created"""

    ####################
    # SepLLM Sparse Attention (ICML 2025, arXiv:2412.12094)
    ####################
    use_sepllm_attention: bool = False
    """Enable SepLLM sparse attention: attend only to initial tokens,
    separator tokens (punctuation/whitespace), and a local sliding window."""

    sepllm_separator_token_ids: Optional[List[int]] = None
    """Token IDs of separator tokens (e.g. '.', ',', '?', '!', ';', ':', ' ', '\\t', '\\n').
    Must be set when use_sepllm_attention=True."""

    sepllm_init_token_count: int = 3
    """Number of initial tokens (attention sinks) to always attend to."""

    sepllm_local_window_size: int = 64
    """Size of the local sliding window for neighboring token attention."""

    sepllm_padding_token_id: int = 0
    """Token ID used for padding."""

    sepllm_kernel: str = 'auto'
    """Kernel backend for SepLLM sparse attention computation.
    'auto': pick best available (flex_attention > triton > dense).
    'flex_attention': PyTorch FlexAttention (>= 2.5) block-sparse kernel.
    'triton': custom Triton fused kernel with block-level skip.
    'dense': materialise full mask, no CUDA-level speedup (fallback)."""
    ...
    def __post_init__(self):
        ...
        # SepLLM validation
        if self.use_sepllm_attention:
            if self.sepllm_separator_token_ids is None or len(self.sepllm_separator_token_ids) == 0:
                raise ValueError(
                    "sepllm_separator_token_ids must be set when use_sepllm_attention=True."
                )
            assert self.sepllm_init_token_count >= 0, (
                "sepllm_init_token_count must be >= 0"
            )
            assert self.sepllm_local_window_size > 0, (
                "sepllm_local_window_size must be > 0"
            )

        # NSA validation
        if self.use_native_sparse_attention:
            assert self.nsa_compress_block_sliding_stride <= self.nsa_compress_block_size, (
                f"nsa_compress_block_sliding_stride ({self.nsa_compress_block_sliding_stride}) "
                f"must be <= nsa_compress_block_size ({self.nsa_compress_block_size})"
            )
            assert self.nsa_compress_block_sliding_stride > 0, (
                "nsa_compress_block_sliding_stride must be > 0"
            )
            assert self.nsa_num_compressed_mem_kv > 0, (
                "nsa_num_compressed_mem_kv must be > 0"
            )
            assert self.nsa_swa_window_size >= 0, (
                f"nsa_swa_window_size ({self.nsa_swa_window_size}) must be >= 0"
            )
            if self.nsa_swa_window_size > 0:
                assert not self.no_rope_freq, (
                    "nsa_swa_window_size > 0 requires RoPE on all layers. "
                    "Set no_rope_freq=None (do not skip RoPE) when using NSA SWA."
                )
            if self.nsa_swa_only:
                assert self.nsa_swa_window_size > 0, (
                    "nsa_swa_only=True requires nsa_swa_window_size > 0."
                )
            assert not self.multi_latent_attention, (
                "NSA and MLA cannot be used together."
            )
