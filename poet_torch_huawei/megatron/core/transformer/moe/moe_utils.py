...
# MaxVio metric tracker: stores tokens_per_expert per layer for global-batch-level MaxVio
# See https://arxiv.org/abs/2408.15664v1 Section 4.1
...
def z_loss_func(logits, z_loss_coeff):
    """Modified z-loss that encourages router logits to stay close to a uniform distribution.

    Original ST-MoE z-loss (https://arxiv.org/pdf/2202.08906.pdf) penalises
    ``logsumexp(logits)`` directly, which pushes all logits toward ``-inf``.
    Here we subtract ``log(num_experts)`` — the logsumexp value that a perfect
    uniform distribution would produce — so that the penalty is zero when the
    router assigns equal probability to every expert and grows only when the
    distribution deviates from uniform.  This encourages balanced routing
    without unnecessarily shrinking the overall logit scale.

    Args:
        logits (torch.Tensor): Router logits of shape [num_tokens, num_experts].
        z_loss_coeff (float): Coefficient for z-loss.

    Returns:
        torch.Tensor: Scalar z-loss value.
    """
    num_experts = logits.shape[1]
    mean_value = torch.log(torch.full((), num_experts, dtype=logits.dtype, device=logits.device))
    z_loss = torch.mean(torch.square(torch.logsumexp(logits, dim=-1) - mean_value)) * z_loss_coeff
    return z_loss
...
def get_tokens_per_expert_tracker():
    """Return the tokens-per-expert tracker for MaxVio computation."""
    global _MOE_TOKENS_PER_EXPERT_TRACKER
    return _MOE_TOKENS_PER_EXPERT_TRACKER


def save_reduced_tokens_for_max_vio(
    stacked_tokens_per_expert: torch.Tensor,
    layer_numbers: List[int],
    num_layers: int,
    num_experts: int,
):
    """Save already TP×CP×DP all-reduced tokens_per_expert for MaxVio computation.

    Called from _update_router_expert_bias in finalize_model_grads.py, which has
    already performed the all-reduce across TP×CP×DP ranks. This avoids a redundant
    all-reduce in the logging path.

    Tracker design:
    To avoid allocating a persistent dense [num_layers, num_experts] tensor for
    sparse/non-uniform MoE placement, we keep per-layer vectors in a dict during
    training and materialize a dense tensor only inside compute_and_track_max_vio.

    Args:
        stacked_tokens_per_expert: Already all-reduced token counts,
            shape [num_local_moe_layers, num_experts].
        layer_numbers: List of 1-indexed global layer numbers for each row.
        num_layers: Total number of layers (including MTP if applicable).
        num_experts: Number of routed experts.
    """
    tracker = get_tokens_per_expert_tracker()
    if "tokens_per_expert_by_layer" not in tracker:
        tracker["tokens_per_expert_by_layer"] = {}
        tracker["num_layers"] = num_layers
        tracker["num_experts"] = num_experts
    tracker["tp_cp_dp_reduced"] = True
    for i, layer_number in enumerate(layer_numbers):
        tracker["tokens_per_expert_by_layer"][int(layer_number)] = stacked_tokens_per_expert[
            i
        ].detach().float()


def save_tokens_per_expert_for_max_vio(
    tokens_per_expert: torch.Tensor,
    layer_number: int,
    num_layers: int,
    num_experts: int,
):
    """Accumulate tokens_per_expert for MaxVio when expert_bias is disabled.

    In this fallback path, data has NOT been all-reduced yet, so
    compute_and_track_max_vio will perform the full all-reduce.

    Args:
        tokens_per_expert: Token counts per expert, shape [num_experts].
        layer_number: 1-indexed global layer number.
        num_layers: Total number of layers (including MTP if applicable).
        num_experts: Number of routed experts.
    """
    if layer_number is None:
    ...
    if layer_number not in tracker["tokens_per_expert_by_layer"]:
        tracker["tokens_per_expert_by_layer"][layer_number] = torch.zeros(
            num_experts, device=tokens_per_expert.device, dtype=torch.float32
        )
    tracker["tokens_per_expert_by_layer"][layer_number] += tokens_per_expert.detach().float()


def clear_tokens_per_expert_tracker():
    """Clear the tokens-per-expert tracker after logging."""
    tracker = get_tokens_per_expert_tracker()
    tracker.pop("tokens_per_expert_by_layer", None)
    tracker.pop("num_layers", None)
    tracker.pop("num_experts", None)
    tracker.pop("tp_cp_dp_reduced", None)


def compute_and_track_max_vio(
    iteration: int,
    writer,
    wandb_writer=None,
    total_loss_dict=None,
    ...
    """Compute and log the MaxVio metric at the global batch level.

    MaxVio = (max_i Load_i - mean_i Load_i) / mean_i Load_i

    When expert_bias is enabled, the TP×CP×DP all-reduce is already done in
    _update_router_expert_bias (via save_reduced_tokens_for_max_vio), so only
    PP all-reduce is needed here.

    When expert_bias is disabled (fallback), the full PP + TP×CP×DP all-reduce
    is performed here.

    Timing: This function is called from training_log() which runs after
    finalize_model_grads(). The PP all-reduce here is safe because all
    pipeline stages have finished their forward-backward pass by this point.
    Under virtual-pipeline (interleaved) schedules each stage writes to
    disjoint layer indices, so the sum-reduce produces correct global counts.

    Reference: https://arxiv.org/abs/2408.15664v1 Section 4.1, Eq. (4)
    """
    tracker = get_tokens_per_expert_tracker()
    if "tokens_per_expert_by_layer" not in tracker:
        return

    by_layer = tracker["tokens_per_expert_by_layer"]
    if len(by_layer) == 0:
        return

    first_tensor = next(iter(by_layer.values()))
    tracked_num_layers = tracker.get("num_layers", num_layers)
    tracked_num_experts = tracker.get("num_experts", first_tensor.shape[0])
    if tracked_num_layers is None:
        return

    # Materialize dense [num_layers, num_experts] only at logging time.
    # This keeps iteration-time memory usage low when MoE layers are sparse.
    ...
    for layer_number, layer_tokens in by_layer.items():
        tokens_per_expert[layer_number - 1] = layer_tokens
    already_reduced = tracker.get("tp_cp_dp_reduced", False)

    # Collect from all pipeline stages (each stage only has its own layers, others are zero)
    torch.distributed.all_reduce(
        tokens_per_expert,
    ...
    if not already_reduced:
        # Fallback: expert_bias is disabled, need full TP×CP×DP all-reduce
        torch.distributed.all_reduce(
            tokens_per_expert,
            group=parallel_state.get_tensor_and_data_parallel_group(with_context_parallel=True),
        )

    # Identify MoE layers (non-zero mean load) to avoid dividing by zero for dense layers
    mean_load = tokens_per_expert.mean(dim=-1)  # [num_layers]
    max_load = tokens_per_expert.max(dim=-1)[0]  # [num_layers]
    moe_layer_mask = mean_load > 0
    max_vio_per_layer = torch.zeros_like(mean_load)
    if moe_layer_mask.any():
        max_vio_per_layer[moe_layer_mask] = (
            (max_load[moe_layer_mask] - mean_load[moe_layer_mask]) / mean_load[moe_layer_mask]
        )
        avg_max_vio = max_vio_per_layer[moe_layer_mask].mean().item()
    else:
        avg_max_vio = 0.0

    if total_loss_dict is not None:
        if "max_vio_global_batch" not in total_loss_dict:
            total_loss_dict["max_vio_global_batch"] = torch.tensor(
                [avg_max_vio], dtype=torch.float, device="cuda"
            )
        else:
            total_loss_dict["max_vio_global_batch"] += avg_max_vio

    if writer is not None:
        writer.add_scalar("max_vio_global_batch", avg_max_vio, iteration)
        if per_layer_logging:
            for i, vio in enumerate(max_vio_per_layer.tolist()):
                if vio > 0:
                    writer.add_scalar(f"moe/max_vio_layer_{i}", vio, iteration)

    if wandb_writer is not None:
        wandb_writer.log({"max_vio_global_batch": avg_max_vio}, iteration)
        if per_layer_logging:
            wandb_writer.log(
                {
                    f"moe/max_vio_layer_{i}": vio
                    for i, vio in enumerate(max_vio_per_layer.tolist())
                    if vio > 0
                },
                iteration,
...
class RouterGatingLinearFunction(torch.autograd.Function):
    ...
    def forward(ctx, inp: torch.Tensor, weight: torch.Tensor, router_dtype: torch.dtype):
        ...
        if HAVE_TE and te_general_gemm is not None and router_dtype != torch.float64:
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        if HAVE_TE and te_general_gemm is not None and ctx.router_dtype != torch.float64:
