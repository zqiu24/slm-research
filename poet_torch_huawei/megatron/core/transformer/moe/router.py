...
from megatron.core.transformer.moe.moe_utils import (
    ...
    save_tokens_per_expert_for_max_vio,
    sinkhorn,
...
class TopKRouter(Router):
    ...
    def __init__(
        ...
        if self.enable_expert_bias:
            self.register_buffer(
                ...
                torch.zeros(self.config.num_moe_experts, dtype=torch.int32),
                persistent=False,
            )
            self.register_buffer(
    ...
    def routing(self, logits: torch.Tensor):
        ...
        if getattr(self, '_monitor_enabled', False):
            self._routing_logits = logits.detach()
        ...
        if self.enable_expert_bias and torch.is_grad_enabled():
            ...
            # When expert_bias is disabled, independently accumulate tokens_per_expert
            # for MaxVio monitoring. When expert_bias is enabled, MaxVio piggybacks on
            # local_tokens_per_expert via _update_router_expert_bias in finalize_model_grads.
            with torch.no_grad():
                num_layers = self.config.num_layers
                if self.config.mtp_num_layers is not None:
                    num_layers += self.config.mtp_num_layers
