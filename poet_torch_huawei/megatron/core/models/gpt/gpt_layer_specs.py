...
from megatron.core.transformer.sepllm_dot_product_attention import SepLLMDotProductAttention
...
def get_gpt_layer_with_transformer_engine_spec(
    ...
    use_native_sparse_attention: Optional[bool] = False,
    fp8: Optional[str] = None,  # pylint: disable=unused-argument
    moe_use_legacy_grouped_gemm: Optional[bool] = False,
    qk_l2_norm: Optional[bool] = False,
    use_te_op_fuser: Optional[bool] = False,
    use_kitchen: bool = False,
    use_sepllm_sparse_kernel: bool = False,
    ...
    if use_native_sparse_attention:
        assert not multi_latent_attention, "NSA and MLA cannot be used together."
        return ModuleSpec(
            module=TransformerLayer,
            submodules=TransformerLayerSubmodules(
                input_layernorm=backend.layer_norm(),
                self_attention=ModuleSpec(
                    module=NSASelfAttention,
                    params={"attn_mask_type": AttnMaskType.causal},
                    submodules=NSASelfAttentionSubmodules(
                        linear_qkv=backend.column_parallel_linear(),
                        compress_layernorm=IdentityOp,
                        compress_mid_norm=backend.layer_norm(),
                        compress_linear_k_1=backend.linear(),
                        compress_linear_k_2=backend.linear(),
                        compress_linear_v_1=backend.linear(),
                        compress_linear_v_2=backend.linear(),
        ...
        core_attn = (
            ModuleSpec(module=SepLLMDotProductAttention)
...
def get_gpt_decoder_block_spec(
    ...
    """GPT block spec."""
    _sepllm_on = getattr(config, 'use_sepllm_attention', False)
    _sepllm_kernel = getattr(config, 'sepllm_kernel', 'dense')
    _head_dim = getattr(config, 'kv_channels', 128)
    # D <= 128: single-tile Triton kernel (GQA expansion materialised by caller).
    # 128 < D <= 384: D-chunked Triton kernel with native GQA support is implemented
    # and correct (see sepllm_kernels.py), but at head_dim=384 the Triton kernel is
    # ~3.5x slower per fwd+bwd than TE FlashAttention (52 vs 15 ms at B=4,H=16,S=4096
    # on L20X). The ~89% mask sparsity cannot close this 3.5x kernel-efficiency gap,
    # so enabling it here regresses training from 33s to 47s per iter. Gate remains
    # D <= 128; to unlock D > 128 you'd need a Triton/CUDA kernel that matches TE FA
    # throughput (hard: TE FA is years of tuning + warp specialisation + cp.async).
    use_sepllm_sparse_kernel = (
        _sepllm_on and _sepllm_kernel != 'dense' and _head_dim <= 128
    )
    ...
    if use_transformer_engine:
        ...
        dense_layer_spec = get_gpt_layer_with_transformer_engine_spec(
            ...
            use_native_sparse_attention=getattr(config, "use_native_sparse_attention", False),
