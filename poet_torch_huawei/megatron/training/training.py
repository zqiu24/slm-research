...
from megatron.training.monitor import monitor_param_grad, set_router_monitor, monitor_router_logits
from megatron.training.nethook import TraceTrainingStats
from megatron.legacy.data.data_samplers import build_pretraining_data_loader
from megatron.core.optimizer_param_scheduler import OptimizerParamScheduler
from megatron.core.transformer.moe import upcycling_utils
from megatron.core.transformer.moe.moe_utils import compute_and_track_max_vio, track_moe_metrics
...
def get_optimizer_param_scheduler(optimizer):
    ...
    opt_param_scheduler = OptimizerParamScheduler(
        ...
        lr_decay_step_ratio=args.lr_decay_step_ratio,
        lr_decay_step_coeff=args.lr_decay_step_coeff,
    )
...
def setup_model_and_optimizer(
    ...
    # Setup stability monitors if enabled
    from megatron.training.stability_utils import setup_stability_monitors
    setup_stability_monitors(model, args)
...
def train_step(forward_step_func, data_iterator, model, optimizer, opt_param_scheduler, config, forward_backward_func):
    ...
    monitor_interval = args.monitor_interval if args.monitor_interval else args.save_interval
    should_monitor = args.enable_monitor and args.curr_iteration % monitor_interval == 0

    if should_monitor:
        set_router_monitor(model, enabled=True)
    ...
    while rerun_state_machine.should_run_forward_backward(data_iterator):
        ...
        # Forward pass.
        with TraceTrainingStats(model=model) if should_monitor else nullcontext():
    ...
    if should_monitor:
        monitor_param_grad(model)
        monitor_router_logits(model)
        set_router_monitor(model, enabled=False)
...
def training_log(
    ...
    if iteration % args.log_interval == 0:
        ...
        # Log stability monitoring metrics
        from megatron.training.stability_utils import log_stability_metrics, get_stability_log_string
        stability_metrics = log_stability_metrics(iteration, args, writer, wandb_writer)
        stability_log_str = get_stability_log_string(stability_metrics)
        if stability_log_str:
            log_string += f' {stability_log_str} |'
...
def train(
    ...
    while iteration < args.train_iters:
        ...
        # Update stability monitors (enable/disable based on sampling frequency)
        from megatron.training.stability_utils import update_stability_monitors
        update_stability_monitors(iteration, args)
