...
def add_megatron_arguments(parser: argparse.ArgumentParser):
    ...
    parser = _add_stability_monitor_args(parser)
...
def validate_args(args, defaults={}):
    ...
    if args.enable_monitor:
        assert args.save is not None, (
            "When using --enable-monitor, the --save option must be specified "
            "so monitor outputs can be persisted."
        )
    ...
    if args.use_sandwich_norm and args.add_bias_linear:
        warn_rank_0(
            "Using --use-sandwich-norm without --disable-bias-linear can change sandwich-norm behavior "
            "because output bias is added after the post-norm path. "
            "Recommended: set --disable-bias-linear.",
            args.rank,
        )
...
def _add_network_size_args(parser):
    ...
    group.add_argument('--use-sandwich-norm', action='store_true',
                       help='Add post-layernorm after attn/mlp output before '
                       'residual connection (sandwich norm).')
    group.add_argument('--attn-pre-norm-scale', type=float, default=1.0,
                       help='Scale for attn pre-layernorm weight init.')
    group.add_argument('--attn-post-norm-scale', type=float, default=1.0,
                       help='Scale for attn post-layernorm weight init.')
    group.add_argument('--ffn-pre-norm-scale', type=float, default=1.0,
                       help='Scale for ffn pre-layernorm weight init.')
    group.add_argument('--ffn-post-norm-scale', type=float, default=1.0,
                       help='Scale for ffn post-layernorm weight init.')
    # Native Sparse Attention (NSA)
    group.add_argument('--use-native-sparse-attention', action='store_true',
                       help='Use Native Sparse Attention (NSA) with compressed '
                       'attention for long-context models.')
    group.add_argument('--nsa-compress-block-size', type=int, default=64,
                       help='Block size for coarse-grained token compression in NSA.')
    group.add_argument('--nsa-compress-block-sliding-stride', type=int, default=32,
                       help='Stride for compression block sliding in NSA. '
                       'Must be <= nsa-compress-block-size.')
    group.add_argument('--nsa-num-compressed-mem-kv', type=int, default=4,
                       help='Number of learnable compressed memory KV slots '
                       '(global tokens) in NSA.')
    group.add_argument('--nsa-compress-mlp-expand-factor', type=float, default=2.0,
                       help='Hidden expansion factor for compression MLP in NSA.')
    group.add_argument('--nsa-swa-window-size', type=int, default=0,
                       help='Sliding-window size for NSA hybrid (compress KV + local '
                       'RoPE KV). 0 = compress-only attention.')
    group.add_argument('--nsa-swa-only', action='store_true', default=False,
                       help='Use sliding-window attention only (no compression). '
                       'Requires --nsa-swa-window-size > 0.')
    # SepLLM Sparse Attention (arXiv:2412.12094)
    group.add_argument('--use-sepllm-attention', action='store_true', default=False,
                       help='Enable SepLLM sparse attention: attend only to initial '
                       'tokens, separator tokens, and a local sliding window.')
    group.add_argument('--sepllm-separator-token-ids', nargs='+', type=int, default=None,
                       help='Token IDs of separator tokens for SepLLM '
                       '(e.g., periods, commas, spaces, newlines).')
    group.add_argument('--sepllm-init-token-count', type=int, default=3,
                       help='Number of initial tokens (attention sinks) for SepLLM.')
    group.add_argument('--sepllm-local-window-size', type=int, default=64,
                       help='Local sliding window size for SepLLM.')
    group.add_argument('--sepllm-padding-token-id', type=int, default=0,
                       help='Padding token ID for SepLLM.')
    group.add_argument('--sepllm-kernel', type=str, default='auto',
                       choices=['auto', 'flex_attention', 'triton', 'dense'],
                       help='Kernel backend for SepLLM sparse attention. '
                       'auto picks best available (flex_attention > triton > dense).')
...
def _add_training_args(parser):
    ...
    group.add_argument('--muon-matched-adamw-rms', type=float, default=0.2,
                       help="Target AdamW update RMS that Muon is scaled to match "
                            "(typically 0.2 ~ 0.4).")
    group.add_argument('--muon-momentum', type=float, default=0.95,
                       help='Momentum beta for Muon (internal SGD momentum).')
    ...
    group.add_argument('--no-muon-nesterov', action='store_false',
                       ...
                       help='Disable Nesterov momentum for Muon.')
    group.add_argument('--muon-ns-num-head-groups', type=int, default=-1,
                       help='For MLA q_up / kv_up projections: split attention heads '
                            'into this many groups before running Newton-Schulz. '
                            '-1 disables the override (use flat 2D shape).')
...
def _add_learning_rate_args(parser):
    ...
    group.add_argument('--lr-decay-style', type=str, default='linear',
                       choices=['constant', 'linear', 'cosine', 'inverse-square-root', 'WSD', 'step'],
    ...
    group.add_argument('--lr-decay-step-ratio', nargs='+', type=float, default=None,
                       help='List of training progress ratios (0~1) at which '
                       'lr steps down. Must be sorted ascending. '
                       'Used with --lr-decay-style step. '
                       'Example: --lr-decay-step-ratio 0.8 0.9')
    group.add_argument('--lr-decay-step-coeff', nargs='+', type=float, default=None,
                       help='List of lr coefficient values at each step point. '
                       'len must equal len of --lr-decay-step-ratio. '
                       'Used with --lr-decay-style step. '
                       'Example: --lr-decay-step-coeff 0.316 0.1')
...
def _add_data_args(parser):
    ...
    group.add_argument(
        ...
        action='store_true',
        dest='simple_dataset',
        help='Use the simple GPT dataset pipeline: concatenate datasets in listed order without '
             'blending/shuffling and validate each sample length against --seq-length + 1.'
    )
    return parser
...
def _add_stability_monitor_args(parser):
    """Add stability monitoring arguments."""
    group = parser.add_argument_group(title='stability monitoring')

    # Transformer Stats Monitor
    group.add_argument('--enable-transformer-stats-monitor', action='store_true',
                       help='Enable transformer stats monitoring. Tracks residual '
                       'velocity, cosine similarity, and variance growth across layers.')

    # Attention Stats Monitor
    group.add_argument('--enable-attention-stats-monitor', action='store_true',
                       help='Enable attention stats monitoring. Tracks attention '
                       'output characteristics using proxy metrics.')

    # Input Token Monitor
    group.add_argument('--enable-input-token-monitor', action='store_true',
                       help='Enable input token monitoring. Tracks entropy and stats '
                       'of input token distribution.')

    # Common settings
    group.add_argument('--stability-monitor-freq', type=int, default=100,
                       help='Frequency of stability metric computation (in training steps).')

    group.add_argument('--stability-monitor-sample-tokens', type=int, default=256,
                       help='Number of tokens to sample for metric computation.')

    group.add_argument('--stability-log-per-layer', action='store_true',
                       help='Log per-layer metrics in addition to aggregated metrics. '
                       'This can be verbose for models with many layers.')
    group.add_argument('--stability-monitor-debug', action='store_true',
                       help='Enable debug prints for stability monitor internals.')

    # Anomaly thresholds for monitors
    group.add_argument('--check-stability-anomalies', action='store_true',
                       help='Check for all anomalies from enabled monitors during training.')

    group.add_argument('--stability-velocity-threshold', type=float, default=1.5,
                       help='Warning threshold for feature velocity. '
                       'Values above this indicate large residual updates.')

    group.add_argument('--stability-cos-sim-threshold', type=float, default=0.5,
                       help='Warning threshold for cosine similarity. '
                       'Values below this indicate trajectory disruption.')

    group.add_argument('--stability-var-growth-threshold', type=float, default=2.0,
                       help='Warning threshold for variance growth. '
                       'Values above this indicate potential variance explosion.')
    return parser


def _add_monitor_args(parser):
    """Add parameter/gradient/activation monitoring arguments."""
    group = parser.add_argument_group(title='training monitor')

    group.add_argument('--enable-monitor', action='store_true', default=False,
                       help='Enable parameter, gradient and activation monitoring.')
    group.add_argument('--monitor-interval', type=int, default=None,
                       help='Monitoring interval in training iterations. '
                       'Defaults to save-interval if not specified.')
    group.add_argument('--monitor-values', type=str, default='mean,std,top1,top10%',
                       help='Comma-separated list of statistics to monitor. '
                       'Supported: mean, std, top1, top10%%.')
    group.add_argument('--monitor-log', action='store_true', default=False,
                       help='Print monitoring statistics to stdout via print_rank_0.')
    return parser
