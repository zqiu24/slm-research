...
# https://github.com/wandb/wandb/blob/main/wandb/sdk/artifacts/_validators.py
_WANDB_ARTIFACT_NAME_MAX_LEN = 128
...
def _truncate_wandb_artifact_name(name: str) -> str:
    """W&B rejects artifact names longer than 128 characters."""
    if len(name) <= _WANDB_ARTIFACT_NAME_MAX_LEN:
        return name
    digest = hashlib.sha256(name.encode()).hexdigest()[:8]
    sep = "_"
    keep = _WANDB_ARTIFACT_NAME_MAX_LEN - len(sep) - len(digest)
    if keep < 1:
        return digest[:_WANDB_ARTIFACT_NAME_MAX_LEN]
    return f"{name[:keep]}{sep}{digest}"
...
def on_save_checkpoint_success(checkpoint_path: str, tracker_filename: str, save_dir: str, iteration: int) -> None:
    ...
    if wandb_writer:
        ...
        artifact_name = _truncate_wandb_artifact_name(artifact_name)
