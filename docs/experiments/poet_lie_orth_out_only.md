# poet_lie_orth_out_only

Pure one-sided POET (output side). `OutOnlyPOETXLinear` trains **only** `oft_R_out` for
the whole run via `optim.poet.single_step_x_one_sided: out`; `oft_R_in` stays at its
