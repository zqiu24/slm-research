...
Split into **four per-shape scripts**, each sweeping its own operating-norm knob at the champion recipe (lr 4e-3/eff∠0.016, varying only init): **_normalized** init_scale {0.5,0.7,1.0,1.4,2.0} (row_rms 0.022→0.088; 1.0 = champion anchor 3.5160; 1.4 brackets the Muon equilibrium); **_normal** (raw, init_type=none) init_scale {1.0,1.5,2.75,4.0,5.5} (native 0.016→0.088; the 2.75 cell ≈ row_rms 0.044 is a matched-norm A/B vs normalized@1.0); **_semiortho** (init_type=orthogonal, κ=1) init_scale {0.5,0.7,1.0,1.4,2.0} (1.0 = matched-norm pure-conditioning A/B vs normalized@1.0); **_mup** (init_type=mup_normalized) mup_alpha {0.25,0.5,1.0,2.0,4.0} (spectral-norm parameterization of the norm axis; init_scale=1.0). Plumbed end-to-end (launcher arg → megatron_args → setup config-copy → apply patch → `_copy_and_init_weight`); 3 CPU tests green ([test_poet_layers.py](/lustre/fast/fast/zqiu/slm-research/tests/unit/test_poet_layers.py): orthogonal κ≈1 + matched RMS, init_scale linear-norm/shape-invariant, normalized unit-row). Hypotheses to resolve: (1) does POET want a *higher* init norm (scale >1) to reach the free-optimizer equilibrium it can't grow into? (2) does an equal-σ (`orthogonal`) base beat `normalized` at matched norm → is *conditioning* a lever beyond scale?
