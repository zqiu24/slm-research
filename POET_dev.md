...
|---|---|---|---|---|---|---|---|---|
| 1 | **nGPT (architecture, lr 1e-2)** | **3.4583** | 31.76 | 3.3573 | **1e-2** | — | — | **🏆 NEW BEST OVERALL** (`ngpt_lr100`/`5zycv3p5`) — normalized-GPT *architecture* (not an optimizer/PEFT method); beats tuned dense adam by **−0.035**, best POET by **−0.065**. Adam-matched recipe (warmup 0.01, wd 0.1, cosine min_lr 0.1) |
| 2 | adam (dense, lr 3e-3) | 3.4935 | 32.90 | 3.3935 | **3e-3** | — | — | best **optimizer/PEFT on llama3** (`ebndt1qj`) — re-tuned dense baseline; −0.035 behind nGPT, beats best POET by **−0.030**, muon_kimi by **−0.039** |
| 3 | poet_lie_orth (+alt, no-head, lr4e-3/c8) | 3.5231 | 33.89 | 3.4233 | **4e-3** | 8 (**0.016**) | **no** | **best POET / best PEFT** (`ghsu7t8y`, cosine grid) — beats muon_kimi by −0.009; behind dense adam by +0.030 |
| 4 | poet_lie_orth (+alt, no-head, lr3e-3/c12) | 3.5274 | 34.04 | 3.4277 | 3e-3 | 12 (0.018) | no | `owcyd976` — angle 0.018 also stable+strong (old doc wrongly called 0.018 divergent) |
| 5 | poet_lie_orth (+alt, no-head, lr4e-3/s0.25/c12) | 3.5288 | 34.08 | 3.4278 | 4e-3 | 12 (0.012) | no | `q60mrt7u` — at angle 0.012, dense-lr 4e-3 beats 3e-3 (hotter dense helps) |
| 6 | muon_kimi | 3.5321 | 34.20 | 3.4219 | 1e-3 | — | — | muon dense baseline (run `of4bakqd`) |
...
| 8 | poet_lie_orth (c8, no-head, both-sides) | 3.5528 | 34.91 | 3.4557 | 3e-3 | 8 (0.012) | no | both-sides head-off (`dwynpk9y`; fresh rerun `f4f49v4f` = 3.5504) |
| 9 | adam (dense, lr 1e-3 — old baseline) | 3.5570 | 35.06 | 3.4575 | 1e-3 | — | — | under-tuned; lr 3e-3 (#2) is −0.064 better (`ylrd45af`) |
| 10 | poet_lie_orth (c8, head) | 3.5667 | 35.40 | 3.4693 | 3e-3 | 8 (0.012) | yes | head-aligned twin (`7lncmww7`, distributed=true) |
| 11 | muon_hybrid | 3.5698 | 35.51 | 3.4705 | — | — | — | |
| 12 | poet_lie_orth (c4) | 3.5715 | 35.57 | 3.4701 | 3e-3 | 4 (0.006) | yes | run `z1gpz9y7` |
| 13 | poet_lie_rms | 3.6193 | 37.31 | 3.5220 | 3e-3 | 4 (rms) | no | best RMS-family (`98293d1u`; head-aligned twin `l2pzawa4` 3.6335 — worse) |
| 14 | poet_dense_rms (c8) | 3.6344 | 37.88 | 3.5367 | 1e-3 | 8 (rms) | no | |
| 15 | poet_lie_rms (c8) | 3.6404 | 38.11 | 3.5367 | 1e-3 | 8 (rms) | no | |
| 16 | poet_lie | 3.6474 | 38.37 | 3.5437 | 1e-3 | — | no | Stage 1 |
| 17 | poet_lie_rms (c4) | 3.6496 | 38.46 | 3.5478 | 1e-3 | 4 (rms) | no | same as #13 but lr 1e-3 |
| 18 | poet0 | 3.6518 | 38.55 | 3.5484 | 1e-3 | — | no | |
| 19 | **poet_h_noperm_rms_c8** | 3.6536 | 38.61 | 3.5578 | 1e-3 | 8 (rms) | **yes** | best head-aligned (RMS family) |
| 20 | poet_h_rms_c8 | 3.6541 | 38.63 | 3.5588 | 1e-3 | 8 (rms) | yes | |
| 21 | poet (vanilla, cayley) | ≈3.70 | ≈40.6 | ≈3.60 | 1e-3 | — | no | weakest POET family |
...
- **nGPT (the normalized-GPT *architecture*) now LEADS overall — below dense adam, POET, and muon, which are all optimizer/PEFT methods on the same llama3 backbone.** The completed nGPT lr sweep ([sweep_ngpt_lr.sh](/lustre/fast/fast/zqiu/slm-research/scripts/sweep_ngpt_lr.sh), adam-matched recipe: warmup 0.01, wd 0.1, cosine min_lr 0.1) peaks at **lr 1e-2 → val 3.4583** (`ngpt_lr100`/`5zycv3p5`, ppl 31.76; lr90 next at 3.4645, so the optimum is bracketed). That **beats tuned dense adam (3.4935) by −0.035** and the best POET (3.5231) by **−0.065** on the identical 60m/40tpp cohort and identical cosine min_lr 0.1 schedule. ⚠️ this is a **cross-architecture** comparison — nGPT changes the model (hypersphere-normalized weights/activations + learned scaling), not just the optimizer, so it sits on a different axis than the adam/muon/POET rows; single seed; the *reference*-recipe nGPT sweep (wd 0, no warmup, [sweep_ngpt_lr_reference.sh](/lustre/fast/fast/zqiu/slm-research/scripts/sweep_ngpt_lr_reference.sh)) is still running and so far trails this recipe (lr5–50 all ≈0.01–0.04 worse).
- **Among optimizer/PEFT methods on llama3, re-tuned dense adam (lr 3e-3) LEADS — POET is the best PEFT method but not the best on this axis either.** A fine-grained adam lr sweep found **lr 3e-3 → val 3.4935** (`ebndt1qj`, ppl 32.90), a **−0.064** jump over the old adam baseline (3.5570 @ lr 1e-3) that beats the best POET (3.5231) by **−0.030** and muon_kimi (3.5321) by **−0.039**. The earlier "POET is outright best" headline was an artifact of comparing tuned POET against an under-tuned (lr 1e-3) dense baseline; with matched lr, dense adam is strongest among same-arch optimizers. ⚠️ adam ran with cosine **min_lr_ratio 0.1** (matching nGPT here, not POET's 0.01) and has not been swept on min_lr / 2–3 seeds yet.
...
**🏆 Overall best (60m/40tpp) — now the nGPT architecture:** [`ngpt_lr100-…-20260617T150127Z`](/lustre/fast/fast/zqiu/slm-research/runs/ngpt_lr100-llama3-60m-s42-20260617T150127Z) (W&B `5zycv3p5`) — **val/loss 3.4583, ppl 31.76**, train 3.3573, 9155 steps. The **normalized-GPT architecture** (hypersphere-normalized weights/activations + learned `alpha/sqk/suv/sz` scaling, `ngpt_adamw`) at **lr 1e-2**, with the optimizer recipe matched to the dense-adam baseline (warmup 0.01, wd 0.1, betas 0.9/0.95, cosine **min_lr_ratio 0.1**) so the only confound vs adam is the architecture. Sweep minimum (lr90 = 3.4645 next), so the optimum is bracketed. **Beats tuned dense adam (3.4935) by −0.035** and the best POET (3.5231) by −0.065 on the identical cohort + schedule. ⚠️ **cross-architecture comparison** — nGPT changes the model, not just the optimizer; single seed; the reference-recipe nGPT sweep (wd 0 / no warmup) is still running and so far trails this recipe. Command:
```bash
...
|---|---|---|---|---|
| **nGPT (architecture)** | [ngpt_lr100-…20260617T150127Z](/lustre/fast/fast/zqiu/slm-research/runs/ngpt_lr100-llama3-60m-s42-20260617T150127Z) (`5zycv3p5`) | **3.4583** | 31.76 | **lr 1e-2**, adam-matched recipe (warmup 0.01, wd 0.1, cosine min_lr 0.1) — **🏆 overall best** (cross-arch; lr sweep min) |
| **adam (dense, tuned)** | [adam_lr30-…20260609T112229Z](/lustre/fast/fast/zqiu/slm-research/runs/adam_lr30-llama3-60m-s42-20260609T112229Z) (`ebndt1qj`) | 3.4935 | 32.90 | **lr 3e-3**, cosine min_lr 0.1, wd 0.1 — **best optimizer/PEFT on llama3** (old lr-1e-3 baseline `ylrd45af` = 3.5570) |
