...
| **Muon-like orthogonalizing Q-opt** | `optim.poet.q_optimizer=lie_ortho`, `lie_ortho_c`/`_method`/`_ns_steps` (`poet_lie_orth`) | standalone `LieOrthMomentum`: same Lie 1st-moment momentum, but **orthogonalize** the skew direction (all planes → ~same angle) instead of RMS-scaling; `muon` band (~5 NS steps) or `spectral` exact `A(−A²)^{-1/2}` (~20) ([poet_lie_orth.py:27](/lustre/fast/fast/zqiu/slm-research/src/optim/poet_lie_orth.py#L27)) | ✅ | **BEST OVERALL** (**3.5231** @ lr **4e-3** / scale 0.5 / c8 → **eff∠ 0.016**, head-OFF + `lie_alternating`, `ghsu7t8y`) — **beats muon_kimi 3.5321 by −0.009**; the 2026-06-09 grid found a hotter optimum (angle 0.016 > old 0.012, dense lr 4e-3 > 3e-3). Prior champ 3.5332 @ eff∠0.012 (`1ynrrimu`) |
...
|---|---|---|---|---|---|---|---|---|
| 1 | **poet_lie_orth (+alt, no-head, lr4e-3/c8)** | **3.5231** | 33.89 | 3.4233 | **4e-3** | 8 (**0.016**) | **no** | **🏆 NEW BEST OVERALL** (`ghsu7t8y`, cosine grid) — beats muon_kimi by **−0.009**; higher angle 0.016 > old 0.012 |
| 2 | poet_lie_orth (+alt, no-head, lr3e-3/c12) | 3.5274 | 34.04 | 3.4277 | 3e-3 | 12 (0.018) | no | `owcyd976` — angle 0.018 also stable+strong (old doc wrongly called 0.018 divergent) |
| 3 | poet_lie_orth (+alt, no-head, lr4e-3/s0.25/c12) | 3.5288 | 34.08 | 3.4278 | 4e-3 | 12 (0.012) | no | `q60mrt7u` — at angle 0.012, dense-lr 4e-3 beats 3e-3 (hotter dense helps) |
| 4 | **muon_kimi** | 3.5321 | 34.20 | 3.4219 | 1e-3 | — | — | best **non-POET** (run `of4bakqd`) |
| 5 | poet_lie_orth (+alt, no-head, lr3e-3/c8) | 3.5332 | 34.23 | 3.4334 | 3e-3 | 8 (0.012) | no | prior best POET (`1ynrrimu`); reproduced by `li3sflwl`, `wj68pgey` |
| 6 | poet_lie_orth (c8, no-head, both-sides) | 3.5528 | 34.91 | 3.4557 | 3e-3 | 8 (0.012) | no | both-sides head-off (`dwynpk9y`; fresh rerun `f4f49v4f` = 3.5504) |
| 7 | adam (baseline) | 3.5570 | 35.06 | 3.4575 | 1e-3 | — | — | best dense baseline |
| 8 | poet_lie_orth (c8, head) | 3.5667 | 35.40 | 3.4693 | 3e-3 | 8 (0.012) | yes | head-aligned twin (`7lncmww7`, distributed=true) |
| 9 | muon_hybrid | 3.5698 | 35.51 | 3.4705 | — | — | — | |
| 10 | poet_lie_orth (c4) | 3.5715 | 35.57 | 3.4701 | 3e-3 | 4 (0.006) | yes | run `z1gpz9y7` |
| 11 | poet_lie_rms | 3.6193 | 37.31 | 3.5220 | 3e-3 | 4 (rms) | no | best RMS-family (`98293d1u`; head-aligned twin `l2pzawa4` 3.6335 — worse) |
| 12 | poet_dense_rms (c8) | 3.6344 | 37.88 | 3.5367 | 1e-3 | 8 (rms) | no | |
| 13 | poet_lie_rms (c8) | 3.6404 | 38.11 | 3.5367 | 1e-3 | 8 (rms) | no | |
| 14 | poet_lie | 3.6474 | 38.37 | 3.5437 | 1e-3 | — | no | Stage 1 |
| 15 | poet_lie_rms (c4) | 3.6496 | 38.46 | 3.5478 | 1e-3 | 4 (rms) | no | same as #11 but lr 1e-3 |
| 16 | poet0 | 3.6518 | 38.55 | 3.5484 | 1e-3 | — | no | |
| 17 | **poet_h_noperm_rms_c8** | 3.6536 | 38.61 | 3.5578 | 1e-3 | 8 (rms) | **yes** | best head-aligned (RMS family) |
| 18 | poet_h_rms_c8 | 3.6541 | 38.63 | 3.5588 | 1e-3 | 8 (rms) | yes | |
| 19 | poet (vanilla, cayley) | ≈3.70 | ≈40.6 | ≈3.60 | 1e-3 | — | no | weakest POET family |
...
- **POET is now the OUTRIGHT BEST at 60m/40tpp — the 2026-06-09 lr×scale×c grid found a hotter optimum.** Best POET (poet_lie_orth + `lie_alternating`, head-OFF, distributed, **lr 4e-3 / scale 0.5 / c8 → eff∠ 0.016**) hits **val 3.5231** (`ghsu7t8y`), **beating muon_kimi (3.5321) by −0.009** and the prior champion (3.5332 @ lr3e-3/eff∠0.012) by −0.010. The long-standing POET gap is not just closed — POET now leads. The stack that got here: head-OFF (−0.014) → `lie_alternating` (−0.017) → **tune the angle up to ~0.016 + dense lr up to 4e-3 (−0.010)**. ⚠️ single seed; worth a 2–3 seed confirm.
...
|---|---|---|---|---|
| **poet_lie_orth (+alt, tuned)** | [cos_lr4_s50_c8-…20260609T080009Z](/lustre/fast/fast/zqiu/slm-research/runs/cos_lr4_s50_c8-llama3-60m-s42-20260609T080009Z) (`ghsu7t8y`) | **3.5231** | 33.89 | **lr 4e-3, scale 0.5, c=8 (eff∠ 0.016)**, muon, head-off, distributed, `lie_alternating=true` — **🏆 overall best** |
| muon_kimi | [muon_kimi-…20260605T142324Z](/lustre/fast/fast/zqiu/slm-research/runs/muon_kimi-llama3-60m-s42-20260605T142324Z) (`of4bakqd`) | 3.5321 | 34.20 | lr 1e-3 — best non-POET |
