...
| **Muon-like orthogonalizing Q-opt** | `optim.poet.q_optimizer=lie_ortho`, `lie_ortho_c`/`_method`/`_ns_steps` (`poet_lie_orth`) | standalone `LieOrthMomentum`: same Lie 1st-moment momentum, but **orthogonalize** the skew direction (all planes → ~same angle) instead of RMS-scaling; `muon` band (~5 NS steps) or `spectral` exact `A(−A²)^{-1/2}` (~20) ([poet_lie_orth.py:27](/lustre/fast/fast/zqiu/slm-research/src/optim/poet_lie_orth.py#L27)) | ✅ | **BEST mainline-confirmed POET / best PEFT** (**3.5231** @ lr **4e-3** / scale 0.5 / c8 → **eff∠ 0.016**, head-OFF + `lie_alternating`, `ghsu7t8y`) — legacy Nesterov+b1.95 candidate hit **3.5152** but needs rerun; mainline champ sits behind **re-tuned muon_kimi** (3.4514, +0.072), **nGPT** (3.4583, +0.065), and **re-tuned dense adam** (**3.4935** @ lr 3e-3, `ebndt1qj`, +0.030); the 2026-06-09 grid found a hotter optimum (angle 0.016 > old 0.012, dense lr 4e-3 > 3e-3). Prior champ 3.5332 @ eff∠0.012 (`1ynrrimu`) |
...
| Nesterov look-ahead (`lie_ortho`) | `optim.poet.lie_ortho_nesterov=true` / `--poet-lie-ortho-nesterov` (legacy pre-rename: `optim.poet.lie_nesterov=true`) | orthogonalize the Muon look-ahead direction `(1−b1)·g + b1·m` (= modern Muon's `grad.lerp(m, β)`) instead of the bare first moment `m`; skew/rotation branch only, AdamW untouched ([poet_lie_orth.py:160-176](/lustre/fast/fast/zqiu/slm-research/src/optim/poet_lie_orth.py#L160-L176)) | ✅ | **mixed / promising:** current flag with default `lie_b1=0.9` peaks at **3.5271** (`nest_lr0.006`) and loses matched A/Bs, but legacy `lie_nesterov=true` + `lie_b1=0.95` hit **3.5152** (`ut682296`) — best observed POET candidate; rerun on current main with `lie_ortho_nesterov=true lie_b1=0.95` |
...
|---|---|---|---|---|---|---|---|---|
| 1 | **muon_kimi (lr 4e-3, wd 0.1)** | **3.4514** | 31.54 | 3.3482 | **4e-3** | — | — | **🏆 NEW BEST OVERALL** (`vtw9k55h`) — re-tuned muon dense baseline; **≈tie with nGPT** (−0.007, ~seed-noise), beats tuned dense adam by **−0.042**, mainline POET by **−0.072** (legacy Nesterov candidate by −0.064) |
| 2 | nGPT (architecture, lr 1e-2) | 3.4583 | 31.76 | 3.3573 | **1e-2** | — | — | normalized-GPT *architecture* (`ngpt_lr100`/`5zycv3p5`) — co-best (−0.007 behind muon_kimi); beats tuned adam by −0.035, mainline POET by −0.065 |
| 3 | adam (dense, lr 3e-3) | 3.4935 | 32.90 | 3.3935 | **3e-3** | — | — | re-tuned dense baseline (`ebndt1qj`); −0.042 behind muon_kimi, beats mainline POET by **−0.030** |
| 4 | poet_lie_orth (+alt, no-head, **legacy Nesterov**, lr4e-3/c8/b1.95) | **3.5152** | 33.62 | 3.4148 | **4e-3** | 8 (**0.016**) | **no** | **best observed POET candidate** (`ut682296`, run dir `poet_lie_orth-20260611T085509Z`); legacy `lie_nesterov=true` pre-rename on side branch, same look-ahead math with `lie_b1=0.95` — rerun current `lie_ortho_nesterov=true lie_b1=0.95` before promoting |
| 5 | poet_lie_orth (+alt, no-head, lr4e-3/c8) | 3.5231 | 33.89 | 3.4233 | **4e-3** | 8 (**0.016**) | **no** | **best mainline-confirmed POET / best PEFT** (`ghsu7t8y`, cosine grid); behind dense adam by +0.030 |
| 6 | poet_lie_orth (+alt, no-head, current Nesterov, lr6e-3/c8) | 3.5271 | 34.02 | 3.4264 | **6e-3** | 8 (**0.024**) | no | best current `lie_ortho_nesterov=true` / `lie_b1=0.9` sweep cell (`nest_lr0.006` / `fnuit4pe`); close but still +0.004 vs non-Nesterov champ |
| 7 | poet_lie_orth (+alt, no-head, lr3e-3/c12) | 3.5274 | 34.04 | 3.4277 | 3e-3 | 12 (0.018) | no | `owcyd976` — angle 0.018 also stable+strong (old doc wrongly called 0.018 divergent) |
| 8 | poet_lie_orth (+alt, no-head, lr4e-3/s0.25/c12) | 3.5288 | 34.08 | 3.4278 | 4e-3 | 12 (0.012) | no | `q60mrt7u` — at angle 0.012, dense-lr 4e-3 beats 3e-3 (hotter dense helps) |
| 9 | muon_kimi (lr 1e-3 — old baseline) | 3.5321 | 34.20 | 3.4219 | 1e-3 | — | — | under-tuned; lr 4e-3 + wd 0.1 (#1) is **−0.081** better (`of4bakqd`; `ijq33tle` rerun 3.5251) |
| 10 | poet_lie_orth (+alt, no-head, lr3e-3/c8) | 3.5332 | 34.23 | 3.4334 | 3e-3 | 8 (0.012) | no | prior best POET (`1ynrrimu`); reproduced by `li3sflwl`, `wj68pgey` |
| 11 | poet_lie_orth (c8, no-head, both-sides) | 3.5528 | 34.91 | 3.4557 | 3e-3 | 8 (0.012) | no | both-sides head-off (`dwynpk9y`; fresh rerun `f4f49v4f` = 3.5504) |
| 12 | adam (dense, lr 1e-3 — old baseline) | 3.5570 | 35.06 | 3.4575 | 1e-3 | — | — | under-tuned; lr 3e-3 (#3) is −0.064 better (`ylrd45af`) |
| 13 | poet_lie_orth (c8, head) | 3.5667 | 35.40 | 3.4693 | 3e-3 | 8 (0.012) | yes | head-aligned twin (`7lncmww7`, distributed=true) |
| 14 | muon_hybrid | 3.5698 | 35.51 | 3.4705 | — | — | — | |
| 15 | poet_lie_orth (c4) | 3.5715 | 35.57 | 3.4701 | 3e-3 | 4 (0.006) | yes | run `z1gpz9y7` |
| 16 | poet_lie_rms | 3.6193 | 37.31 | 3.5220 | 3e-3 | 4 (rms) | no | best RMS-family (`98293d1u`; head-aligned twin `l2pzawa4` 3.6335 — worse) |
| 17 | poet_dense_rms (c8) | 3.6344 | 37.88 | 3.5367 | 1e-3 | 8 (rms) | no | |
| 18 | poet_lie_rms (c8) | 3.6404 | 38.11 | 3.5367 | 1e-3 | 8 (rms) | no | |
| 19 | poet_lie | 3.6474 | 38.37 | 3.5437 | 1e-3 | — | no | Stage 1 |
| 20 | poet_lie_rms (c4) | 3.6496 | 38.46 | 3.5478 | 1e-3 | 4 (rms) | no | same as #16 but lr 1e-3 |
| 21 | poet0 | 3.6518 | 38.55 | 3.5484 | 1e-3 | — | no | |
| 22 | **poet_h_noperm_rms_c8** | 3.6536 | 38.61 | 3.5578 | 1e-3 | 8 (rms) | **yes** | best head-aligned (RMS family) |
| 23 | poet_h_rms_c8 | 3.6541 | 38.63 | 3.5588 | 1e-3 | 8 (rms) | yes | |
| 24 | poet (vanilla, cayley) | ≈3.70 | ≈40.6 | ≈3.60 | 1e-3 | — | no | weakest POET family |
...
- **The top of the board is now a near-tie between re-tuned muon_kimi and nGPT, both clearly ahead of POET and even tuned adam.** On the identical 60m/40tpp cohort + cosine min_lr 0.1 schedule: **muon_kimi lr 4e-3 / wd 0.1 → val 3.4514** (`vtw9k55h`, ppl 31.54) and **nGPT lr 1e-2 → 3.4583** (`5zycv3p5`) are within −0.007 (~seed noise), both beating **tuned dense adam (3.4935)** by ~0.035–0.042 and the mainline-confirmed best POET (3.5231) by ~0.065–0.072. So among same-arch (llama3) optimizers, **muon_kimi — not adam — is now strongest**, and POET remains the best PEFT method but not the best overall.
...
- **Tuned dense adam (lr 3e-3 → 3.4935, `ebndt1qj`)** is now 3rd overall — a −0.064 jump over the old lr-1e-3 baseline (3.5570), still beating the mainline-confirmed best POET by −0.030, but **−0.042 behind muon_kimi**. ⚠️ adam ran with cosine **min_lr_ratio 0.1** and has not been swept on min_lr / 2–3 seeds yet.
- **POET is still the best PEFT method and the strongest non-dense optimizer — it just lost the overall crown.** The mainline-confirmed POET champ (poet_lie_orth + `lie_alternating`, head-OFF, distributed, **lr 4e-3 / scale 0.5 / c8 → eff∠ 0.016**) hits **val 3.5231** (`ghsu7t8y`), still **beating the old under-tuned muon_kimi (3.5321) by −0.009** and the prior champion (3.5332 @ lr3e-3/eff∠0.012) by −0.010, but now behind re-tuned muon_kimi (3.4514, +0.072), nGPT (3.4583, +0.065), and dense adam (3.4935, +0.030). A legacy side-branch Nesterov run (`ut682296`, `lie_nesterov=true`, `lie_b1=0.95`) records **3.5152** — the best observed POET candidate — but needs a current-main rerun before replacing `ghsu7t8y`. The POET-internal stack that got here: head-OFF (−0.014) → `lie_alternating` (−0.017) → **tune the angle up to ~0.016 + dense lr up to 4e-3 (−0.010)**.
- **Nesterov is unresolved, not simply a win.** The current `lie_ortho_nesterov=true` sweep with default `lie_b1=0.9` completed: the coupled LR sweep improves monotonically through **lr 6e-3 / eff∠0.024 → 3.5271** (`nest_lr0.006`) but still misses the non-Nesterov champ by +0.004, and the matched decoupled A/B (`decn_*_n1` vs `n0`) loses at every fixed dense-lr/angle cell. The promising signal is specifically the older **`lie_b1=0.95`** run at 3.5152. Next run should be current `lie_ortho_nesterov=true optim.poet.lie_b1=0.95` around lr4/angle0.016 and lr6/angle0.024.
...
Champion-quality setting (**updated 2026-06-14**): **lr 4e-3, scale 0.5, c=8** (eff∠ **0.016**), muon, **head-aligned OFF**, **+ `lie_alternating=true`**, cosine min_lr 0.01. Current mainline-confirmed best completed run is `cos_lr4_s50_c8` (`ghsu7t8y`) = **val/loss 3.5231** — **beats the old under-tuned muon_kimi 3.5321** (but behind the re-tuned muon_kimi 3.4514, `vtw9k55h`) — found by the lr×scale×c grid (arm **F** below). The best observed POET candidate is legacy Nesterov+b1.95 `ut682296` = **3.5152**, pending current-main rerun (§2.5-G.2). The prior champion `1ynrrimu` (lr3e-3/eff∠0.012) = 3.5332 is arm **E**. The head-off **both-sides** run `dwynpk9y` = **3.5528** (the `lieorth_c8_nohead` arm — head-off beats head-on by −0.014; fresh both-sides rerun `f4f49v4f` = 3.5504). Among the *head-aligned* arms, the original anchor `5sbgancm` = **3.5669**, reproduced 4× (`lieorth_c8_muon`, `lieorth_lr0.003`, `lieorth_scale0.5`, + the original anchor), with the distributed rerun `7lncmww7` = **3.5667**.
...
→ **Current-flag Nesterov (`b1=0.9`) is not a winner.** It only edges no-Nesterov at the already-worse 0.024 cells, and loses at the useful 0.012–0.020 band. The one genuinely promising result is the legacy **`b1=0.95`** run at **3.5152**; the clean next step is a same-mainline confirm:
...
| poet_lie_orth (+alt, legacy Nesterov candidate) | [poet_lie_orth-…20260611T085509Z](/lustre/fast/fast/zqiu/slm-research/runs/poet_lie_orth-llama3-60m-s42-20260611T085509Z) (`ut682296`) | **3.5152** | 33.62 | lr 4e-3, scale 0.5, c=8 (eff∠ 0.016), head-off, distributed, `lie_alternating=true`, legacy `lie_nesterov=true`, **`lie_b1=0.95`** — best observed POET candidate; needs current-main rerun |
| poet_lie_orth (+alt, tuned mainline) | [cos_lr4_s50_c8-…20260609T080009Z](/lustre/fast/fast/zqiu/slm-research/runs/cos_lr4_s50_c8-llama3-60m-s42-20260609T080009Z) (`ghsu7t8y`) | 3.5231 | 33.89 | **lr 4e-3, scale 0.5, c=8 (eff∠ 0.016)**, muon, head-off, distributed, `lie_alternating=true` — **best mainline-confirmed POET / best PEFT (4th overall)** |
| poet_lie_orth (+alt, current Nesterov b1=0.9) | [nest_lr0.006-…20260613T122151Z](/lustre/fast/fast/zqiu/slm-research/runs/nest_lr0.006-llama3-60m-s42-20260613T122151Z) (`fnuit4pe`) | 3.5271 | 34.02 | lr 6e-3, scale 0.5, c=8 (eff∠ 0.024), `lie_ortho_nesterov=true`, `lie_b1=0.9`; best current-key Nesterov cell, but +0.004 vs mainline champ |
