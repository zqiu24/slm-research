...
| **Muon-like orthogonalizing Q-opt** | `optim.poet.q_optimizer=lie_ortho`, `lie_ortho_c`/`_method`/`_ns_steps` (`poet_lie_orth`) | standalone `LieOrthMomentum`: same Lie 1st-moment momentum, but **orthogonalize** the skew direction (all planes → ~same angle) instead of RMS-scaling; `muon` band (~5 NS steps) or `spectral` exact `A(−A²)^{-1/2}` (~20) ([poet_lie_orth.py:27](/lustre/fast/fast/zqiu/slm-research/src/optim/poet_lie_orth.py#L27)) | ✅ | **BEST POET / best PEFT** (**3.5160** @ lr **4e-3** / scale 0.5 / c8 → **eff∠ 0.016**, head-OFF + `lie_alternating` + **Nesterov b1.95**, `nestON_lr4`; reproduces legacy `ut682296`=3.5152) — beats the prior non-Nesterov champ 3.5231 (`ghsu7t8y`) by −0.007; still behind **re-tuned muon_kimi** (3.4514, +0.065), **nGPT** (3.4583, +0.058), and **re-tuned dense adam** (**3.4935** @ lr 3e-3, `ebndt1qj`, +0.022); the 2026-06-09 grid found a hotter optimum (angle 0.016 > old 0.012, dense lr 4e-3 > 3e-3), then Nesterov b1.95 added −0.007. Prior champ 3.5332 @ eff∠0.012 (`1ynrrimu`) |
...
|---|---|---|---|---|---|---|---|---|
| 1 | **muon_kimi (lr 4e-3, wd 0.1)** | **3.4514** | 31.54 | 3.3482 | **4e-3** | — | — | **🏆 BEST OVERALL** (`vtw9k55h`) — re-tuned muon dense baseline; **≈tie with nGPT** (−0.007, ~seed-noise), beats tuned dense adam by **−0.042**, best POET (Nesterov b1.95) by **−0.065** |
| 2 | nGPT (architecture, lr 1e-2) | 3.4583 | 31.76 | 3.3573 | **1e-2** | — | — | normalized-GPT *architecture* (`ngpt_lr100`/`5zycv3p5`) — co-best (−0.007 behind muon_kimi); beats tuned adam by −0.035, best POET by −0.058 |
| 3 | **nGPT + muon_kimi** (lr 8e-3) | **3.4882** | 32.73 | 3.3932 | **8e-3** | — | — | nGPT *architecture* trained with Muon ([`ngpt_muon_lr80`](/lustre/fast/fast/zqiu/slm-research/scripts/sweep_ngpt_muon_lr.sh), flat optimum lr 6–8e-3). **Anti-synergy:** worse than BOTH nGPT-adam (#2, +0.030) and dense-muon (#1, +0.037) — the two wins cancel; only −0.005 ahead of dense adam (#4). See [ngpt_muon.md](/lustre/fast/fast/zqiu/slm-research/docs/experiments/ngpt_muon.md) |
| 4 | adam (dense, lr 3e-3) | 3.4935 | 32.90 | 3.3935 | **3e-3** | — | — | re-tuned dense baseline (`ebndt1qj`); −0.042 behind muon_kimi, beats best POET by **−0.022** |
| 5 | poet_lie_orth (+alt, no-head, **Nesterov b1.95**, lr4e-3/c8) — legacy | **3.5152** | 33.62 | 3.4148 | **4e-3** | 8 (**0.016**) | **no** | 🥇 **NEW BEST POET / best PEFT** recipe (`ut682296`, side-branch `lie_nesterov`); **CONFIRMED on current main** by `nestON_lr4`=3.5160 (#6) — this is the recipe now baked into config defaults |
| 6 | poet_lie_orth (+alt, no-head, **Nesterov b1.95**, lr4e-3/c8) — current main | **3.5160** | 33.65 | 3.4218 | **4e-3** | 8 (**0.016**) | **no** | **current-main confirmation** (`nestON_lr4`, `lie_ortho_nesterov=true lie_b1=0.95`); reproduces legacy 3.5152 (Δ0.0008), beats the non-Nesterov champ 3.5231 by **−0.0071**. Deconfound: nestOFF b1.95 best = 3.5247 ≈ b1=0.9 champ → the gain is the look-ahead, NOT the b1 bump |
| 7 | poet_lie_orth (+alt, no-head, Nesterov b1.95, lr3e-3/c8) | 3.5208 | 33.81 | 3.4255 | 3e-3 | 8 (0.012) | no | `nestON_lr3` — 2nd-best Nesterov cell, also beats the non-Nesterov champ (3.5231) |
| 8 | poet_lie_orth (+alt, no-head, lr4e-3/c8) | 3.5231 | 33.89 | 3.4233 | **4e-3** | 8 (**0.016**) | **no** | prior **non-Nesterov** champ (`ghsu7t8y`, cosine grid); superseded by Nesterov b1.95 (#5/#6) by −0.007 |
| 9 | poet_lie_orth (+alt, no-head, current Nesterov **b1=0.9**, lr6e-3/c8) | 3.5271 | 34.02 | 3.4264 | **6e-3** | 8 (**0.024**) | no | best `lie_b1=0.9` Nesterov cell (`nest_lr0.006` / `fnuit4pe`); b1=0.9 misses by +0.004 — the **b1=0.95** bump is what makes Nesterov a win (#5/#6) |
| 10 | poet_lie_orth (+alt, no-head, lr3e-3/c12) | 3.5274 | 34.04 | 3.4277 | 3e-3 | 12 (0.018) | no | `owcyd976` — angle 0.018 also stable+strong (old doc wrongly called 0.018 divergent) |
| 11 | poet_lie_orth (+alt, no-head, lr4e-3/s0.25/c12) | 3.5288 | 34.08 | 3.4278 | 4e-3 | 12 (0.012) | no | `q60mrt7u` — at angle 0.012, dense-lr 4e-3 beats 3e-3 (hotter dense helps) |
| 12 | muon_kimi (lr 1e-3 — old baseline) | 3.5321 | 34.20 | 3.4219 | 1e-3 | — | — | under-tuned; lr 4e-3 + wd 0.1 (#1) is **−0.081** better (`of4bakqd`; `ijq33tle` rerun 3.5251) |
| 13 | poet_lie_orth (+alt, no-head, lr3e-3/c8) | 3.5332 | 34.23 | 3.4334 | 3e-3 | 8 (0.012) | no | prior best POET (`1ynrrimu`); reproduced by `li3sflwl`, `wj68pgey` |
| 14 | poet_lie_orth (c8, no-head, both-sides) | 3.5528 | 34.91 | 3.4557 | 3e-3 | 8 (0.012) | no | both-sides head-off (`dwynpk9y`; fresh rerun `f4f49v4f` = 3.5504) |
| 15 | adam (dense, lr 1e-3 — old baseline) | 3.5570 | 35.06 | 3.4575 | 1e-3 | — | — | under-tuned; lr 3e-3 (#4) is −0.064 better (`ylrd45af`) |
| 16 | poet_lie_orth (c8, head) | 3.5667 | 35.40 | 3.4693 | 3e-3 | 8 (0.012) | yes | head-aligned twin (`7lncmww7`, distributed=true) |
| 17 | muon_hybrid | 3.5698 | 35.51 | 3.4705 | — | — | — | |
| 18 | poet_lie_orth (c4) | 3.5715 | 35.57 | 3.4701 | 3e-3 | 4 (0.006) | yes | run `z1gpz9y7` |
| 19 | poet_lie_rms | 3.6193 | 37.31 | 3.5220 | 3e-3 | 4 (rms) | no | best RMS-family (`98293d1u`; head-aligned twin `l2pzawa4` 3.6335 — worse) |
| 20 | poet_dense_rms (c8) | 3.6344 | 37.88 | 3.5367 | 1e-3 | 8 (rms) | no | |
| 21 | poet_lie_rms (c8) | 3.6404 | 38.11 | 3.5367 | 1e-3 | 8 (rms) | no | |
| 22 | poet_lie | 3.6474 | 38.37 | 3.5437 | 1e-3 | — | no | Stage 1 |
| 23 | poet_lie_rms (c4) | 3.6496 | 38.46 | 3.5478 | 1e-3 | 4 (rms) | no | same as #19 but lr 1e-3 |
| 24 | poet0 | 3.6518 | 38.55 | 3.5484 | 1e-3 | — | no | |
| 25 | **poet_h_noperm_rms_c8** | 3.6536 | 38.61 | 3.5578 | 1e-3 | 8 (rms) | **yes** | best head-aligned (RMS family) |
| 26 | poet_h_rms_c8 | 3.6541 | 38.63 | 3.5588 | 1e-3 | 8 (rms) | yes | |
| 27 | poet (vanilla, cayley) | ≈3.70 | ≈40.6 | ≈3.60 | 1e-3 | — | no | weakest POET family |
...
- **The top of the board is now a near-tie between re-tuned muon_kimi and nGPT, both clearly ahead of POET and even tuned adam.** On the identical 60m/40tpp cohort + cosine min_lr 0.1 schedule: **muon_kimi lr 4e-3 / wd 0.1 → val 3.4514** (`vtw9k55h`, ppl 31.54) and **nGPT lr 1e-2 → 3.4583** (`5zycv3p5`) are within −0.007 (~seed noise), both beating **tuned dense adam (3.4935)** by ~0.035–0.042 and the best POET (3.5160, Nesterov b1.95) by ~0.058–0.065. So among same-arch (llama3) optimizers, **muon_kimi — not adam — is now strongest**, and POET remains the best PEFT method but not the best overall.
...
- **Tuned dense adam (lr 3e-3 → 3.4935, `ebndt1qj`)** is now 4th overall (nGPT+Muon edges it by −0.005, within seed noise) — a −0.064 jump over the old lr-1e-3 baseline (3.5570), still beating the best POET (3.5160) by −0.022, but **−0.042 behind muon_kimi**. ⚠️ adam ran with cosine **min_lr_ratio 0.1** and has not been swept on min_lr / 2–3 seeds yet.
- **POET is still the best PEFT method and the strongest non-dense optimizer — it just lost the overall crown.** The POET champ is now poet_lie_orth + `lie_alternating`, head-OFF, distributed, **+ Nesterov look-ahead at `lie_b1=0.95`**, **lr 4e-3 / scale 0.5 / c8 → eff∠ 0.016**, hitting **val 3.5160** (`nestON_lr4`, current main; reproduces the legacy side-branch `ut682296` = 3.5152). This **beats the prior non-Nesterov champ 3.5231 (`ghsu7t8y`) by −0.0071** and the old under-tuned muon_kimi (3.5321) by −0.016, but is still behind re-tuned muon_kimi (3.4514, +0.065), nGPT (3.4583, +0.058), and dense adam (3.4935, +0.022). The POET-internal stack that got here: head-OFF (−0.014) → `lie_alternating` (−0.017) → **angle up to ~0.016 + dense lr up to 4e-3 (−0.010)** → **Nesterov b1.95 (−0.007)**. These flags (`lie_ortho_nesterov=true`, `lie_b1=0.95`) are now the config defaults.
...
Champion-quality setting (**updated 2026-06-23**): **lr 4e-3, scale 0.5, c=8** (eff∠ **0.016**), muon, **head-aligned OFF**, **+ `lie_alternating=true`**, **+ Nesterov look-ahead `lie_ortho_nesterov=true` at `lie_b1=0.95`** (now config defaults), cosine min_lr 0.01. Current best completed run is `nestON_lr4` = **val/loss 3.5160** — reproduces the legacy Nesterov+b1.95 `ut682296` (3.5152) and **beats the prior non-Nesterov champ `ghsu7t8y` (3.5231) by −0.007** (still behind re-tuned muon_kimi 3.4514, `vtw9k55h`). The deconfound A/B (§2.5-G.3) confirms the gain is the look-ahead, not the b1 bump (nestOFF b1.95 best = 3.5247 ≈ champ). The prior non-Nesterov champ `ghsu7t8y` (lr4/scale0.5/c8, eff∠0.016) = 3.5231 was found by the lr×scale×c grid (arm **F** below). The prior champion `1ynrrimu` (lr3e-3/eff∠0.012) = 3.5332 is arm **E**. The head-off **both-sides** run `dwynpk9y` = **3.5528** (the `lieorth_c8_nohead` arm — head-off beats head-on by −0.014; fresh both-sides rerun `f4f49v4f` = 3.5504). Among the *head-aligned* arms, the original anchor `5sbgancm` = **3.5669**, reproduced 4× (`lieorth_c8_muon`, `lieorth_lr0.003`, `lieorth_scale0.5`, + the original anchor), with the distributed rerun `7lncmww7` = **3.5667**.
...
**Preliminary (2026-06-24, 4-GPU grid ~35/60 cells done — single seed, optima at grid edges):** the answer to (1) is a strong YES and to (2) a NO. (a) **Raw init scaled UP is the best so far: `init_none_s400_c6` = 3.4818** (init_scale 4.0 → row_rms 0.064, eff∠0.012) — **−0.034 vs the 3.5160 champion, and it now beats tuned dense adam 3.4935 by −0.012** (preliminary; if confirmed, POET → 3rd overall on this cohort — also −0.006 ahead of the now-committed nGPT+Muon 3.4882, −0.024/−0.030 behind nGPT/muon_kimi). Raw's *native* norm (row_rms 0.016) is its worst cell (3.5585); the win is purely from scaling up. (b) **Every shape improves monotonically with operating norm and none has bottomed out** (none c6: 0.016→3.5585 … 0.064→3.4818 still falling; normalized c6: 0.022→3.5430 → 0.031→3.5323; mup c6: α0.25→3.671 … α2.0→3.5169) — so the champion's `normalized`/scale-1.0 sits *below* its own scale-optimum (under-scaled). (c) **`c6` (eff∠0.012) ≥ c8 ≥ c10 everywhere** → the rotation wants to be cooler than the c8 champion at these norms. (d) **conditioning is NOT the lever: `orthogonal` (κ=1) is the weakest structured shape** (3.56–3.61), and at matched norm 0.044 raw (3.4923) beats it by ~0.07 → the raw residual-structured spectrum, not equal-σ, is what helps. Since all optima are at grid edges, a 3×8-GPU **extension** (one machine each) pushes higher-scale × cooler-angle (`c∈{2,4,6}` = eff∠ 0.004/0.008/0.012; integer +1 scale ladders): [_normal_hi](/lustre/fast/fast/zqiu/slm-research/scripts/sweep_poet_init_normal_hi.sh) (init_scale 4–8), [_normalized_hi](/lustre/fast/fast/zqiu/slm-research/scripts/sweep_poet_init_normalized_hi.sh) (init_scale 1–5), [_mup_hi](/lustre/fast/fast/zqiu/slm-research/scripts/sweep_poet_init_mup_hi.sh) (mup_alpha 2–6), each with an 8-GPU repro of `none_s400_c6` as a 4↔8-GPU parity check. `semiortho` dropped (weakest).
...
**🏆 Overall best (60m/40tpp) — re-tuned muon_kimi:** [`muon_kimi-…-20260609T141524Z`](/lustre/fast/fast/zqiu/slm-research/runs/muon_kimi-llama3-60m-s42-20260609T141524Z) (W&B `vtw9k55h`) — **val/loss 3.4514, ppl 31.54**, train 3.3482, 9155 steps. The Kimi/Muon optimizer (per-block Newton-Schulz orthogonalization + RMS scaling, `muon_momentum=0.95`, Nesterov, `ns_steps=5`) at **lr 4e-3 with weight_decay 0.1**, cosine **min_lr_ratio 0.1**, warmup 0.01. A **−0.081** jump over the old tracker entry (3.5321 @ lr 1e-3) — both higher lr and turning wd on help. **Edges nGPT (3.4583) by −0.007 (≈seed-noise, effectively co-best)** and beats tuned dense adam (3.4935) by **−0.042**, best POET (3.5160, Nesterov b1.95) by −0.065, on the identical cohort + schedule. ⚠️ single seed (no lr 1e-4 run exists — the win is at lr 4e-3); 6e-3 not yet tried. Command:
```bash
codexlog muon_kimi scripts/train_muon_dev.sh optim.lr=0.004 optim.weight_decay=0.1 experiment.name=muon_kimi
```

**🥈 Co-best overall — nGPT architecture:** [`ngpt_lr100-…-20260617T150127Z`](/lustre/fast/fast/zqiu/slm-research/runs/ngpt_lr100-llama3-60m-s42-20260617T150127Z) (W&B `5zycv3p5`) — **val/loss 3.4583, ppl 31.76**, train 3.3573, 9155 steps. The **normalized-GPT architecture** (hypersphere-normalized weights/activations + learned `alpha/sqk/suv/sz` scaling, `ngpt_adamw`) at **lr 1e-2**, optimizer recipe matched to the dense-adam baseline (warmup 0.01, wd 0.1, betas 0.9/0.95, cosine **min_lr_ratio 0.1**). Sweep minimum (lr90 = 3.4645 next), so the optimum is bracketed. Within −0.007 of muon_kimi; **beats tuned dense adam (3.4935) by −0.035**, best POET (3.5160) by −0.058. ⚠️ **cross-architecture comparison** — nGPT changes the model, not just the optimizer; single seed; the reference-recipe nGPT sweep (wd 0 / no warmup) is still running and so far trails this recipe. Command:
```bash
...
**🥉 3rd overall — nGPT + Muon:** [`ngpt_muon_lr80-…-20260623T230232Z`](/lustre/fast/fast/zqiu/slm-research/runs/ngpt_muon_lr80-llama3-60m-s42-20260623T230232Z) — **val/loss 3.4882, ppl 32.73**, train 3.3932, 9155 steps. The **nGPT architecture trained with the Kimi/Muon optimizer** (`optim.type=muon_kimi`, momentum 0.95, nesterov, ns_steps 5) at **lr 8e-3, wd 0.1**, 1% warmup, cosine **min_lr_ratio 0.1** — the muon-matched recipe, flat optimum (lr60 = 3.4884 ≈ tied). Edges tuned dense adam (3.4935) by −0.005 (≈seed noise), but is the **worst of the four {dense,nGPT}×{adam,muon} cells**: +0.037 behind dense-muon and +0.030 behind nGPT-adam — **nGPT and Muon are anti-synergistic** (each helps alone, the gains cancel when stacked; see §2.3 and [docs/experiments/ngpt_muon.md](/lustre/fast/fast/zqiu/slm-research/docs/experiments/ngpt_muon.md)). ⚠️ single seed; `wd=0.1` leg (no `ngpt_optimizer_setup` zero-WD bucketing → decays the nGPT scaling vectors); pure nGPT-reference leg (wd 0 / no-warmup) not yet run. Command:
```bash
...
**4th overall — re-tuned dense adam:** [`adam_lr30-…-20260609T112229Z`](/lustre/fast/fast/zqiu/slm-research/runs/adam_lr30-llama3-60m-s42-20260609T112229Z) (W&B `ebndt1qj`) — **val/loss 3.4935, ppl 32.90**, train 3.3935, 9155 steps. Plain **adamw at lr 3e-3** (betas 0.9/0.95, wd 0.1, eps 1e-8), cosine **min_lr_ratio 0.1**, warmup 0.01. A **−0.064** jump over the old adam baseline (3.5570 @ lr 1e-3, `ylrd45af`); **beats best POET (3.5160) by −0.022** — but now **+0.042 behind muon_kimi**. ⚠️ single seed, and adam has not been swept on min_lr (POET's best used 0.01); worth a 2–3 seed confirm. Command:
```bash
codexlog adam_lr30 bash scripts/train_adam.sh llama3 optim.lr=0.003
```

**🏅 Best POET / best PEFT method (5th overall) — Nesterov b1.95 champion:** `nestON_lr4` (current main, 2026-06-23) — **val/loss 3.5160, ppl 33.65**, train 3.4218, 9155 steps. The head-off `lie_ortho` + alternating champion at **lr 4e-3, scale 0.5, c8 → eff∠ 0.016**, **plus the Muon Nesterov look-ahead (`lie_ortho_nesterov=true`) at `lie_b1=0.95`** — now the config defaults. **Reproduces the legacy side-branch `ut682296` (3.5152, Δ0.0008)** on current main, and **beats the prior non-Nesterov champ `ghsu7t8y` (3.5231) by −0.0071**. The G.3 deconfound proves the gain is the look-ahead, not the b1 bump (b1=0.95 OFF best = 3.5247 ≈ b1=0.9 champ 3.5231). Still −0.065/−0.058/−0.028/−0.022 behind muon_kimi/nGPT/nGPT+Muon/adam. ⚠️ run on 4 GPUs (dp=4, 2× grad-accum, global_batch=1024) vs the 8-GPU `ghsu7t8y`; an 8-GPU repro would lock in the record. Command:
```bash
...
| **nGPT + muon_kimi** | [ngpt_muon_lr80-…20260623T230232Z](/lustre/fast/fast/zqiu/slm-research/runs/ngpt_muon_lr80-llama3-60m-s42-20260623T230232Z) | **3.4882** | 32.73 | **lr 8e-3, wd 0.1**, muon_kimi (momentum 0.95, nesterov, ns_steps 5), cosine min_lr 0.1 — **🥉 3rd overall**; worst of the {dense,nGPT}×{adam,muon} matrix (anti-synergy, §2.3) |
| adam (dense, tuned) | [adam_lr30-…20260609T112229Z](/lustre/fast/fast/zqiu/slm-research/runs/adam_lr30-llama3-60m-s42-20260609T112229Z) (`ebndt1qj`) | 3.4935 | 32.90 | **lr 3e-3**, cosine min_lr 0.1, wd 0.1 — 4th overall (old lr-1e-3 baseline `ylrd45af` = 3.5570) |
| **poet_lie_orth (+alt, Nesterov b1.95)** | `nestON_lr4` (current main, 2026-06-23) | **3.5160** | 33.65 | **lr 4e-3, scale 0.5, c=8 (eff∠ 0.016)**, muon, head-off, distributed, `lie_alternating=true`, **`lie_ortho_nesterov=true`, `lie_b1=0.95`** — **🏅 best POET / best PEFT (5th overall)**; reproduces legacy `ut682296` (3.5152), beats non-Nesterov champ by −0.007 (now config default) |
