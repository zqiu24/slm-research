...
|---|---|---|---|---|---|---|---|---|
| 1 | **muon_kimi (lr 4e-3, wd 0.1)** | **3.4514** | 31.54 | 3.3482 | **4e-3** | — | — | **🏆 BEST OVERALL** (`vtw9k55h`) — re-tuned muon dense baseline; **≈tie with nGPT** (−0.007, ~seed-noise), beats tuned dense adam by **−0.042**, best POET (update-RMS+decorr) by **−0.017** |
| 2 | nGPT (architecture, lr 1e-2) | 3.4583 | 31.76 | 3.3573 | **1e-2** | — | — | normalized-GPT *architecture* (`ngpt_lr100`/`5zycv3p5`) — co-best (−0.007 behind muon_kimi); beats tuned adam by −0.035, best POET by −0.010 |
| 3 | **poet_lie_orth_update_rms + decorrelation (mup α4, side_γ+0.25, λ0.25, `renorm=off`)** | **3.4686** | 32.09 | 3.3722 | **5e-3** | θ=lr·ρ/RMS (ρ0.30) | **no** | 🥇 **BEST POET / best PEFT** (`urms_decorr_gp25_mup_l0p25_rnf`, §2.15c). §J.3 cross-side decorrelation (`lambda=0.25`, `renorm=false`, `mode=symmetric`) on the side_γ+0.25 update-RMS champion. **−0.0059 vs the prior best POET** (update-RMS 3.4745, §2.12), −0.0080 vs the fixed-angle lr5-tuned champion (3.4766, `lrsc_mup_lr5_ps50` §2.10). **Beats tuned adam (3.4935) by −0.025** → POET still 3rd overall, −0.0103/−0.0172 behind nGPT/muon_kimi. `renorm=false` > `renorm=true` (3.4699) at every cell; λ0.25 optimum, λ0.75 cliff. ⚠️ single seed, < noise floor — seed-confirm pending |
| 3b | poet_lie_orth (+alt, no-head, Nesterov b1.95, init_mup α4, lr5/c6) — fixed-angle | 3.4766 | 32.35 | 3.3752 | **5e-3** | 6 (**0.015**) | no | prior fixed-angle champion (`lrsc_mup_lr5_ps50`, §2.10), superseded by #3 (−0.0080) and the update-RMS variant (3.4745, §2.12). At the §2.9 init optimum, dense lr 4e-3→5e-3 at scale 0.5 (eff∠ 0.012→0.015) lowered mup α4 from 3.4816 to 3.4766; **normalized s2 ties at 3.4770** (#4); `init_none scale4`/lr4 cell = 3.4804 (`hi_none_s4_c6`). ⚠️ single seed |
| 4 | poet_lie_orth (+alt, no-head, Nesterov b1.95, init_normalized s2, lr5/c6) | 3.4770 | 32.36 | 3.3772 | **5e-3** | 6 (**0.015**) | no | `lrsc_norm_lr5_ps50` (§2.10) — normalized s2 ties the mup best (Δ0.0004) at dense lr 5e-3 / scale 0.5; both edge the `none`/lr4 cell (3.4804, prior champion). The §2.9 mup/lr4 cell was `init_mup_a400_c6` = 3.4816 |
| 5 | **nGPT + muon_kimi** (lr 8e-3) | **3.4882** | 32.73 | 3.3932 | **8e-3** | — | — | nGPT *architecture* trained with Muon ([`ngpt_muon_lr80`](/lustre/fast/fast/zqiu/slm-research/scripts/sweep_ngpt_muon_lr.sh), flat optimum lr 6–8e-3). **Anti-synergy:** worse than BOTH nGPT-adam (#2, +0.030) and dense-muon (#1, +0.037) — the two wins cancel; +0.020 behind best POET (#3). See [ngpt_muon.md](/lustre/fast/fast/zqiu/slm-research/docs/experiments/ngpt_muon.md) |
| 6 | adam (dense, lr 3e-3) | 3.4935 | 32.90 | 3.3935 | **3e-3** | — | — | re-tuned dense baseline (`ebndt1qj`); −0.042 behind muon_kimi, now **+0.025 behind best POET (update-RMS+decorr)** |
| 7 | poet_lie_orth (+alt, no-head, **Nesterov b1.95**, lr4e-3/c8, default init) — legacy | **3.5152** | 33.62 | 3.4148 | **4e-3** | 8 (**0.016**) | **no** | prior best-POET recipe (`ut682296`, side-branch `lie_nesterov`); confirmed on main by `nestON_lr4`=3.5160 (#8). Now superseded by #3 (−0.047, init-scaled + lr5 + update-RMS/decorr). This is the recipe baked into config defaults (init still `normalized`/scale 1.0) |
| 8 | poet_lie_orth (+alt, no-head, **Nesterov b1.95**, lr4e-3/c8, default init) — current main | **3.5160** | 33.65 | 3.4218 | **4e-3** | 8 (**0.016**) | **no** | **current-main default-init champion** (`nestON_lr4`, `lie_ortho_nesterov=true lie_b1=0.95`); reproduces legacy 3.5152 (Δ0.0008). Deconfound: nestOFF b1.95 best = 3.5247 ≈ b1=0.9 champ → the gain is the look-ahead, NOT the b1 bump. Superseded by #3 (−0.047, init-scaling + lr5 + update-RMS/decorr) |
...
- **The top of the board is a near-tie between re-tuned muon_kimi and nGPT, but POET (init-scaled) has now closed most of the gap and sits 3rd — ahead of both nGPT+Muon and tuned adam.** On the identical 60m/40tpp cohort: **muon_kimi lr 4e-3 / wd 0.1 → val 3.4514** (`vtw9k55h`, ppl 31.54) and **nGPT lr 1e-2 → 3.4583** (`5zycv3p5`) co-lead (within −0.007, ~seed noise). The §2.5-K init sweep jumped the best POET to 3.4804 (`hi_none_s4_c6`), the §2.10 lr×scale sweep lowered it to 3.4766 (`lrsc_mup_lr5_ps50`, mup α4 / dense lr 5e-3), and the **§2.11–§2.15 update-RMS line — self-scaled angle `θ=lr·ρ/RMS(W)` (no `lie_ortho_c`) + in/out `side_γ=+0.25` + cross-side decorrelation (`λ0.25`, `renorm=off`) — took it to 3.4686** (`urms_decorr_gp25_mup_l0p25_rnf`, §2.15c) — which **beats tuned dense adam (3.4935) by −0.025 and nGPT+Muon (3.4882) by −0.020** — so POET is now only ~0.010–0.017 behind the leaders, still the best PEFT method and the **best non-architecture, non-muon optimizer** on this cohort. (The default-init POET champion was 3.5160.) ⚠️ the 3.4686 margin is single-seed and < the ~0.01 noise floor — seed-confirm pending.
...
- **Tuned dense adam (lr 3e-3 → 3.4935, `ebndt1qj`)** is now 5th overall — a −0.064 jump over the old lr-1e-3 baseline (3.5570), but the update-RMS+decorrelation best POET (3.4686) now **beats it by −0.025** and it is **−0.042 behind muon_kimi**. ⚠️ adam ran with cosine **min_lr_ratio 0.1** and has not been swept on min_lr / 2–3 seeds yet.
- **POET is the best PEFT method and now 3rd overall — scaling up the frozen-base init was the biggest single POET lever, the update-RMS angle law + cross-side decorrelation the latest.** The POET champ is now poet_lie_orth + `lie_alternating`, head-OFF, distributed, Nesterov b1.95, **`init_type=mup_normalized` α4** (row_rms ≈0.064), the **update-RMS angle law `θ=lr·ρ/RMS(W)`** (ρ0.30, no `lie_ortho_c`) with in/out **`side_γ=+0.25`**, plus **cross-side decorrelation** (`λ0.25`, `renorm=off`), hitting **val 3.4686** (`urms_decorr_gp25_mup_l0p25_rnf`, §2.15c). This **beats tuned dense adam (3.4935) by −0.025 and nGPT+Muon (3.4882) by −0.020**, leaving it only +0.010/+0.017 behind nGPT/muon_kimi. The POET-internal stack that got here: head-OFF (−0.014) → `lie_alternating` (−0.017) → angle/dense-lr up (−0.010) → Nesterov b1.95 (−0.007, → 3.5160 default-init champ) → **init scaled up + cooler angle (−0.035, → 3.4804)** → **§2.10 dense lr 5e-3 at scale 0.5 (−0.004, → 3.4766)** → **§2.11–12 update-RMS angle + `side_γ+0.25` (−0.002, → 3.4745)** → **§2.15c cross-side decorrelation (−0.006, → 3.4686)**. None of these champion knobs are config defaults yet (the default still ships `normalized`/scale-1.0 = 3.5160); the whole stack is single-seed and < the ~0.01 noise floor, so a 2–3-seed confirm is the pending step.
...
**🏆 Overall best (60m/40tpp) — re-tuned muon_kimi:** [`muon_kimi-…-20260609T141524Z`](/lustre/fast/fast/zqiu/slm-research/runs/muon_kimi-llama3-60m-s42-20260609T141524Z) (W&B `vtw9k55h`) — **val/loss 3.4514, ppl 31.54**, train 3.3482, 9155 steps. The Kimi/Muon optimizer (per-block Newton-Schulz orthogonalization + RMS scaling, `muon_momentum=0.95`, Nesterov, `ns_steps=5`) at **lr 4e-3 with weight_decay 0.1**, cosine **min_lr_ratio 0.1**, warmup 0.01. A **−0.081** jump over the old tracker entry (3.5321 @ lr 1e-3) — both higher lr and turning wd on help. **Edges nGPT (3.4583) by −0.007 (≈seed-noise, effectively co-best)** and beats tuned dense adam (3.4935) by **−0.042**, best POET (3.4686, update-RMS+decorr) by −0.017, on the identical cohort + schedule. ⚠️ single seed (no lr 1e-4 run exists — the win is at lr 4e-3); 6e-3 not yet tried. Command:
```bash
codexlog muon_kimi scripts/train_muon_dev.sh optim.lr=0.004 optim.weight_decay=0.1 experiment.name=muon_kimi
```

**🥈 Co-best overall — nGPT architecture:** [`ngpt_lr100-…-20260617T150127Z`](/lustre/fast/fast/zqiu/slm-research/runs/ngpt_lr100-llama3-60m-s42-20260617T150127Z) (W&B `5zycv3p5`) — **val/loss 3.4583, ppl 31.76**, train 3.3573, 9155 steps. The **normalized-GPT architecture** (hypersphere-normalized weights/activations + learned `alpha/sqk/suv/sz` scaling, `ngpt_adamw`) at **lr 1e-2**, optimizer recipe matched to the dense-adam baseline (warmup 0.01, wd 0.1, betas 0.9/0.95, cosine **min_lr_ratio 0.1**). Sweep minimum (lr90 = 3.4645 next), so the optimum is bracketed. Within −0.007 of muon_kimi; **beats tuned dense adam (3.4935) by −0.035**, best POET (3.4686, update-RMS+decorr) by −0.010. ⚠️ **cross-architecture comparison** — nGPT changes the model, not just the optimizer; single seed; the reference-recipe nGPT sweep (wd 0 / no warmup) is still running and so far trails this recipe. Command:
```bash
...
## 2.12 update-RMS in/out side-asymmetry — `side_gamma` sweep (complete)
...
`†` `mup` γ=−0.5 (3.4906, W&B `3lus64z8`) was rerun solo to get this value — the first attempt OOM'd at the final checkpoint save, diagnosed as **GPU co-location** (a concurrent job held 76 GiB on the shared card → <11 MiB free), not a code/save bug (training was clean to iter 9150). Confirms the negative tail: it's the worst `mup` cell, as predicted.
...
- **`mup` → `γ=+0.25`.** A clean valley shifted positive (3.4906 / 3.4842 / 3.4758 / **3.4745** / 3.4757): rotating the wider fc-out side ~1.41× more wins. `mup` `γ=+0.25 = 3.4745` **beats the symmetric §2.11 mup champion (3.4758) by −0.0013 and the fixed-angle champion (3.4766) by −0.0021 → new best POET / best PEFT (§2.6).**
...
|---|---|---|---|---|
| 0 (control) | 3.4745 (= §2.12 ✓) | — | 3.4765 (= §2.12 ✓) | — |
| 5e-5  | 3.4741 | −0.0004 | 3.4761 | −0.0004 |
| 1.5e-4 | **3.4725** | −0.0020 | **3.4751** | −0.0014 |
| 5e-4  | 3.4799 | +0.0054 | 3.4803 | +0.0038 |

**Result — gentle gain is NEUTRAL, ~confirming §2.17's verdict but rehabilitating the mechanism.** Both controls reproduce their §2.12 champions **exactly** (mup 3.4745, norm-s2 3.4765 — same-code anchor solid, and the s2 fix landed: first-pass norm@s1 was off-anchor at 3.5228). Both inits then show the **same shape**: ≈baseline at 5e-5, a shallow dip at **1.5e-4** (−0.0020 mup / −0.0014 norm), back above baseline at 5e-4 (+0.004–0.005). Two readings:
